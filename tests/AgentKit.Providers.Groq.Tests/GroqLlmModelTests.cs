...
using AgentKit.Providers.Groq.Tests.Fakes;

/// <summary>Verifies GroqLlmModel behavior and contracts.</summary>
public sealed class GroqLlmModelTests
{
    ...
    private static ModelDescriptor CreateDescriptor() => new(new ModelAlias("chat"), GroqProviderDefaults.ProviderId, GroqProviderDefaults.ApiFamily, new ModelId("llama-3.3-70b-versatile"), deploymentId: null, GroqProviderDefaults.DefaultCapabilities, GroqProviderDefaults.DefaultLimits, pricing: null, ExtensionData.Empty);
    [Fact]
    public async Task ExecuteAsync_WhenUsingApiKeyCredential_SendsBearerHeaderAndReturnsCompletedResponse()
    {
        ...
        var model = new GroqLlmModel(descriptor, GroqProviderDefaults.CreateProfile(options), new OpenAIRequestTranslator(), new OpenAIChatCompletionResponseParser(new SequentialToolCallIdGenerator()), new StaticApiKeyCredentialSource("real-looking-key"), new HttpClient(handler), new FakeTimeProvider(Now));
        ...
        completed.Response.Parts[0].ShouldBeOfType<TextPart>().Text.ShouldBe("Hello from Groq!");
        completed.Response.Identity.ProviderId.ShouldBe(GroqProviderDefaults.ProviderId);
        _ = handler.Requests.ShouldHaveSingleItem();
        handler.Requests[0].RequestUri.ShouldBe(new Uri("https://api.groq.com/openai/v1/chat/completions"));
    ...
    public async Task ExecuteAsync_WhenUsingExpiredOAuthCredential_FailsAuthenticationWithoutSendingHttpRequest()
    {
        ...
        var model = new GroqLlmModel(descriptor, GroqProviderDefaults.CreateProfile(options), new OpenAIRequestTranslator(), new OpenAIChatCompletionResponseParser(new SequentialToolCallIdGenerator()), new DelegatingOAuthCredentialSource(new StaticOAuthTokenProvider(expiredToken)), new HttpClient(handler), new FakeTimeProvider(Now));
        var observer = new RecordingModelResponseObserver();
        var result = await model.ExecuteAsync(CreateRequest(descriptor), observer, TestContext.Current.CancellationToken);
