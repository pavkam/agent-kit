...
/// <summary>
/// End-to-end tests for the concrete <see cref="AzureOpenAILlmModel"/>,
...
/// real network call to Azure.
/// </summary>
public sealed class AzureOpenAILlmModelEndToEndTests
{
    ...
    private static ModelDescriptor CreateDescriptor() =>
        new(
            ...
            new ModelId("gpt-4o"),
            new DeploymentId("prod-gpt4o"),
            AzureOpenAIProviderDefaults.DefaultCapabilities,
            AzureOpenAIProviderDefaults.DefaultLimits,
            pricing: null,
            ExtensionData.Empty);

    private static AzureOpenAILlmModel CreateModel(
        StubHttpMessageHandler handler,
        IProviderCredentialSource credentials,
        ModelDescriptor descriptor) =>
        new(
            descriptor,
            AzureOpenAIProviderDefaults.CreateProfile(new AzureOpenAIProviderOptions
            {
    ...
    public async Task ExecuteAsync_WhenModelDoesNotSupportRequestedTools_FailsWithoutSendingHttpRequest()
    {
        ...
        {
            Capabilities = AzureOpenAIProviderDefaults.DefaultCapabilities with { SupportsToolCalls = false },
        };
        var model = CreateModel(handler, new StaticApiKeyCredentialSource("azure-resource-key"), descriptor);
    ...
    public async Task ExecuteAsync_WhenCallerCancelsBeforeCredentialResolution_ReturnsCancelledResult()
    {
        ...
        var model = CreateModel(
            handler,
            new StaticProviderCredentialSource(new ApiKeyProviderCredential("azure-resource-key")),
            descriptor);
