...
/// <summary>
/// End-to-end tests for <see cref="MistralAILlmModel"/>, exercising the
...
/// </summary>
public sealed class MistralAILlmModelEndToEndTests
{
    ...
    private static MistralAILlmModel CreateModel(
        ...
        new(
            descriptor ?? TestModels.MistralLarge,
            options ?? new MistralAIProviderOptions { BaseAddress = new Uri("https://api.mistral.test/v1/") },
            new MistralAIRequestTranslator(),
            new MistralAIResponseParser(new SequentialToolCallIdGenerator()),
            credentials,
    ...
    public async Task ExecuteAsync_WhenUnauthenticated_ReturnsAuthenticationFailureWithStatusCodeAndSafeMessage()
    {
        ...
        var options = new MistralAIProviderOptions { BaseAddress = new Uri("https://api.mistral.test/v1/"), PreferStreaming = false };
    ...
    public async Task ExecuteAsync_WhenValidationErrorArray_JoinsFieldMessagesIntoSafeMessage()
    {
        ...
        failed.Failure.SafeMessage.ShouldBe(
            "Input should be 'system', 'user', 'assistant' or 'tool' Input should be less than or equal to 1.5");
    }
    ...
    public async Task ExecuteAsync_WhenModelDoesNotSupportRequestedTools_FailsWithoutSendingHttpRequest()
    {
        ...
        var model = CreateModel(
            handler,
            new StaticProviderCredentialSource(new ApiKeyProviderCredential("mistral-test-key")),
    ...
    public async Task ExecuteAsync_WhenProviderRequestOptionsCarryExtensionData_ForwardsThemInSentBody()
    {
        ...
        var providerOptions = new ProviderRequestOptions(
            new ExtensionData(ImmutableDictionary<string, ExtensionValue>.Empty.Add("prompt_cache_key", cacheKeyValue)));
