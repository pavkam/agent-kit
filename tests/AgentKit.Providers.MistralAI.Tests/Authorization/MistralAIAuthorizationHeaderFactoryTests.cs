...
namespace AgentKit.Providers.MistralAI.Tests.Authorization;

/// <summary>
/// Verifies that <see cref="MistralAIAuthorizationHeaderFactory"/> sends
/// both an API key and an OAuth token as <c>Authorization: Bearer</c>, and
/// rejects an expired OAuth token before any network call would occur.
/// </summary>
public sealed class MistralAIAuthorizationHeaderFactoryTests
{
    private static readonly ProviderId ProviderId = new("mistral-ai");
    ...
    {
        var result = MistralAIAuthorizationHeaderFactory.Create(
            new ApiKeyProviderCredential("mistral-test-key"), ProviderId, Clock);

        var granted = result.ShouldBeOfType<MistralAIAuthorizationGranted>();
    ...
    public void Create_WhenOAuthTokenNotExpired_ReturnsGrantedAuthorizationBearerHeader()
    {
        ...
        var result = MistralAIAuthorizationHeaderFactory.Create(credential, ProviderId, Clock);
    ...
    public void Create_WhenOAuthTokenExpired_ReturnsDeniedAuthenticationFailure()
    {
        ...
        var denied = result.ShouldBeOfType<MistralAIAuthorizationDenied>();
    ...
    public void Create_WhenOAuthTokenHasNoExpiry_ReturnsGranted()
    {
        ...
        _ = result.ShouldBeOfType<MistralAIAuthorizationGranted>();
    }
}
