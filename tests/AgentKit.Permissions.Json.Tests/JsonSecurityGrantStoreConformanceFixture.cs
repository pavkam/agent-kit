...
/// <summary>Creates one isolated initialized JSON store root for each reusable grant-store contract case.</summary>
public sealed class JsonSecurityGrantStoreConformanceFixture: ISecurityGrantStoreConformanceFixture
{
    ...
    private JsonSecurityGrantStore? _store;
    ...
    public async ValueTask<ISecurityGrantStore> CreateAsync(CancellationToken cancellationToken = default)
    {
        if (_store is null)
        {
            ...
            _ = services.AddJsonSecurityGrantStore(
                new JsonSecurityGrantStoreTarget(
                    Path.Combine(_directory, "grants"),
                ...
                JsonSecurityGrantStoreSettings.CreateDefault());
            _provider = services.BuildServiceProvider(validateScopes: true);
            _store = _provider.GetRequiredService<ISecurityGrantStore>().ShouldBeOfType<JsonSecurityGrantStore>();
