...
/// <summary>Verifies JsonApprovalStoreTarget bootstrap validation, normalization, and value semantics.</summary>
public sealed class JsonApprovalStoreTargetTests
{
        ...
        JsonStoreOpenMode openMode, JsonStoreRecoveryMode recoveryMode)
    {
        ...
        var target = new JsonApprovalStoreTarget(path, instanceId, openMode, recoveryMode);
    ...
    public void With_WhenCopiedWithoutChanges_RetainsEveryCoordinate()
    {
        var instanceId = new JsonApprovalStoreInstanceId(Guid.NewGuid());
        var path = TestTemporaryDirectory.Create();
        var original = new JsonApprovalStoreTarget(
