...
public sealed class ZAILlmModelTests
{
    ...
    private static LlmModelRequest CreateRequest(ModelDescriptor descriptor, ImmutableArray<LlmToolDefinition> tools = default, LlmRequestSettings? settings = null)
    {
    ...
    public async Task ExecuteAsync_WhenParallelToolCallsAreRequested_FailsCapabilityValidationBeforeSending()
    {
        ...
        var observer = new RecordingModelResponseObserver();
        var result = await model.ExecuteAsync(CreateRequest(descriptor, tools, settings), observer, TestContext.Current.CancellationToken);
