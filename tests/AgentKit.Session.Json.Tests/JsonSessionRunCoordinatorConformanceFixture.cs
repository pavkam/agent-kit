...
/// <summary>Composes the default local run coordinator over the real protected JSON session transaction path.</summary>
public sealed class JsonSessionRunCoordinatorConformanceFixture:
    ...
    ISecurityAuditDispatcher
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_services is null)
        {
            ...
            await _services.GetRequiredService<ISessionStore>().ShouldBeOfType<JsonSessionStore>()
                .InitializeAsync(cancellationToken).ConfigureAwait(false);
            await _services.GetRequiredService<ISessionDirectory>().ShouldBeOfType<JsonSessionDirectory>()
                .InitializeAsync(cancellationToken).ConfigureAwait(false);
        }

        _grants = _services.GetRequiredService<ISecurityGrantStore>();
        var coordinator = _services.GetRequiredService<ISessionCoordinator>();
        var profile = TestSecurityEvidence.SessionProfile("agentkit.json");
