...
public sealed class InMemorySecurityGrantStoreTests
{
    ...
    public async Task ValidateAndConsumeAsync_WhenIdenticalIntentContends_AuthorizesExactlyOneConsumer()
    {
        ...
        var calls = Enumerable.Range(0, 32).Select(_ => Task.Run(async () =>
            await store.ValidateAndConsumeAsync(grant, enforcement, intent, TestContext.Current.CancellationToken)));
    ...
    public async Task ValidateAndConsumeAsync_WhenConsumedIntentIsReplayedAfterExpiry_ReturnsHistoricalReceiptOnly()
    {
        ...
        var replay = await store.ValidateAndConsumeAsync(grant, enforcement, intent, TestContext.Current.CancellationToken);
        var freshIntent = await store.ValidateAndConsumeAsync(
    ...
    public async Task ValidateAndConsumeAsync_WhenSameIntentChangesEffectOrFence_DoesNotConsumeAnotherUse(bool changeEffect)
    {
        ...
        var changedIntent = changeEffect
            ? intent
            : new SecurityEnforcementIntent(intent.Id, new FencingToken(9));
        ...
        var final = await store.ValidateAndConsumeAsync(
            grant, enforcement, CreateIntent(2), TestContext.Current.CancellationToken);
    ...
    public async Task ValidateAndConsumeAsync_WhenCallerAlreadyCancelled_PreservesOriginalCancellationAndUse()
    {
        ...
        var exception = await Should.ThrowAsync<OperationCanceledException>(async () =>
            await store.ValidateAndConsumeAsync(grant, CreateEnforcement(grant), CreateIntent(), source.Token));
        var later = await store.ValidateAndConsumeAsync(
            grant, CreateEnforcement(grant), CreateIntent(2), TestContext.Current.CancellationToken);
