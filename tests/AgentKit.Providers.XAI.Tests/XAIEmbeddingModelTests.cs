...
/// <summary>Verifies XAIEmbeddingModel behavior and contracts.</summary>
public sealed class XAIEmbeddingModelTests
{
    private static readonly DateTimeOffset Now = new(2025, 6, 1, 12, 0, 0, TimeSpan.Zero);
    private static EmbeddingModelDescriptor CreateDescriptor() => new(new EmbeddingModelAlias("embed"), XAIProviderDefaults.ProviderId, XAIProviderDefaults.EmbeddingApiFamily, new ModelId("xai-embed-1"), deploymentId: null, XAIProviderDefaults.DefaultEmbeddingCapabilities, XAIProviderDefaults.DefaultEmbeddingLimits, pricing: null, ExtensionData.Empty);
    ...
    public async Task GenerateAsync_WhenUsingApiKeyCredential_SendsBearerHeaderAndReturnsCompletedResponse()
    {
        ...
        var model = new XAIEmbeddingModel(descriptor, XAIProviderDefaults.CreateProfile(options), new OpenAIEmbeddingRequestTranslator(), new OpenAIEmbeddingResponseParser(), new StaticApiKeyCredentialSource("xai-real-looking-key"), new HttpClient(handler), new FakeTimeProvider(Now));
