...
internal sealed class FakeFileGlobber: IFileGlobber
{
    public ComponentId SecurityAudience { get; } = new("test.globber");

    public List<GlobRequest> Requests { get; } = [];

    public GlobResult Result { get; set; } = new(GlobStatus.NoMatches, [], 0, true, null);
