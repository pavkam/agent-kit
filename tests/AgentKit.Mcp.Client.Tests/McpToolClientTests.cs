...
public sealed class McpToolClientTests
{
    ...
    public async Task CallAsync_WhenObserved_EmitsContentFreeCorrelatedActivities()
    {
        ...
        var caller = new FakeToolCaller(
            ...
            [WeatherRemoteTool()],
            JsonSerializer.SerializeToElement(new WeatherResponse(responseSecret)));
        await using var client = await McpToolClientActivator.CreateAsync<WeatherTools>(
            caller,
            cancellationToken: TestContext.Current.CancellationToken);
        using var parent = new Activity("test-parent").Start();

        _ = await client.CallAsync(
            tools => tools.GetAsync(new WeatherRequest(requestSecret), default),
            TestContext.Current.CancellationToken);
        parent.Stop();

        var activity = stopped.Single(item =>
            item.OperationName == AgentKitActivityNames.McpToolCall
        ...
        var diagnosticText = string.Join(
            '|',
            stopped.SelectMany(static item => item.TagObjects)
    ...
    public async Task CallAsync_WhenContractMatches_SendsReflectedNameAndObject()
    {
        var caller = new FakeToolCaller(
            McpProtocolVersions.July2026,
            [new McpRemoteTool(new McpToolName("weather.get"), new ToolVersion("2.1"))],
            JsonSerializer.SerializeToElement(new WeatherResponse("sunny")));
        await using var client = await McpToolClientActivator.CreateAsync<WeatherTools>(
            caller,
            cancellationToken: TestContext.Current.CancellationToken);
        ...
        var result = await client.CallAsync(
            tools => tools.GetAsync(request, default),
            TestContext.Current.CancellationToken);
    ...
    public async Task CreateAsync_WhenRemoteVersionDiffers_RejectsBeforeInvocation()
    {
        var caller = new FakeToolCaller(
            McpProtocolVersions.July2026,
            [new McpRemoteTool(new McpToolName("weather.get"), new ToolVersion("1.0"))],
            JsonSerializer.SerializeToElement(new WeatherResponse("unused")));

        var exception = await Should.ThrowAsync<McpToolContractMismatchException>(
            async () => await McpToolClientActivator.CreateAsync<WeatherTools>(
                caller,
                cancellationToken: TestContext.Current.CancellationToken));
    ...
    public async Task CreateAsync_WhenRemoteCatalogHasAdditionalTools_PublishesOnlyContractMatches()
    {
        var caller = CreateCaller([
            WeatherRemoteTool(),
            new McpRemoteTool(new McpToolName("unrelated"), new ToolVersion("9"))
        ]);
    ...
    public async Task RefreshAsync_WhenLaterCatalogIsInvalid_PreservesPublishedSnapshotAndGeneration()
    {
        ...
        _ = await Should.ThrowAsync<McpToolContractMismatchException>(
            async () => await client.RefreshAsync(TestContext.Current.CancellationToken));
    ...
    public async Task RefreshAsync_WhenCancellationIsRequested_PropagatesWithoutPublishing()
    {
        ...
        _ = await Should.ThrowAsync<OperationCanceledException>(
            async () => await client.RefreshAsync(cancellation.Token));
    ...
    public async Task CallAsync_WhenValueTaskMethodSelected_InvokesAndDeserializesResponse()
    {
        var caller = new FakeToolCaller(
            McpProtocolVersions.July2026,
            [new McpRemoteTool(new McpToolName("weather.value-task"), new ToolVersion("1"))],
            JsonSerializer.SerializeToElement(new WeatherResponse("cloudy")));
        await using var client = await McpToolClientActivator.CreateAsync<ValueTaskTools>(
            caller,
            cancellationToken: TestContext.Current.CancellationToken);

        var result = await client.CallAsync(
            tools => tools.GetAsync(new WeatherRequest("Porto"), default),
            TestContext.Current.CancellationToken);
    ...
    public async Task CallAsync_WhenRequestComesFromClosure_PreservesObjectIdentity()
    {
        ...
        _ = await client.CallAsync(
            tools => tools.GetAsync(holder.Request, default),
            TestContext.Current.CancellationToken);
    ...
    public async Task CallAsync_WhenExpressionIsNotDirectMethodCall_RejectsBeforeRemoteCall()
    {
        var caller = new FakeToolCaller(
            McpProtocolVersions.July2026,
            [new McpRemoteTool(new McpToolName("property.execute"), new ToolVersion("1"))],
            JsonSerializer.SerializeToElement(new WeatherResponse("unused")));
        await using var client = await McpToolClientActivator.CreateAsync<PropertyTools>(
            caller,
            cancellationToken: TestContext.Current.CancellationToken);
    ...
    public async Task CallAsync_WhenExpressionCallsForeignMethod_RejectsBeforeRemoteCall()
    {
        ...
        var exception = await Should.ThrowAsync<ArgumentException>(
            async () => await client.CallAsync(
                tools => Forward(tools, new WeatherRequest("Faro")),
                TestContext.Current.CancellationToken));
    ...
    public async Task CallAsync_WhenStructuredContentIsJsonNull_RejectsNonNullResponse()
    {
        ...
        var exception = await Should.ThrowAsync<JsonException>(
            async () => await client.CallAsync(
                tools => tools.GetAsync(new WeatherRequest("Braga"), default),
                TestContext.Current.CancellationToken));
    ...
    public async Task CallAsync_WhenCallerFails_PropagatesFailureWithoutRetry()
    {
        ...
        var exception = await Should.ThrowAsync<InvalidOperationException>(
            async () => await client.CallAsync(
                tools => tools.GetAsync(new WeatherRequest("Evora"), default),
                TestContext.Current.CancellationToken));
    ...
    public async Task CallAsync_WhenCustomSerializerProvided_UsesPrivateEquivalentOptions()
    {
        ...
        _ = await client.CallAsync(
            tools => tools.GetAsync(new WeatherRequest("Aveiro"), default),
            TestContext.Current.CancellationToken);
    ...
    {
        var caller = CreateCaller([WeatherRemoteTool()]);
        var client = await McpToolClientActivator.CreateAsync<WeatherTools>(
            caller,
            cancellationToken: TestContext.Current.CancellationToken);
        ...
        _ = await Should.ThrowAsync<ObjectDisposedException>(
            async () => await client.CallAsync(
                tools => tools.GetAsync(new WeatherRequest("Leiria"), default),
                TestContext.Current.CancellationToken));
    ...
    public void RequireAtLeast_WhenVersionIsUninitialized_ThrowsForVersion()
    {
        var exception = Should.Throw<ArgumentOutOfRangeException>(
            () => McpClientVersionPolicy.RequireAtLeast(default));

        exception.ParamName.ShouldBe("version");
    }

    [Fact]
    public void Automatic_WhenRead_HasNoMinimumVersion() =>
        McpClientVersionPolicy.Automatic.MinimumVersion.ShouldBeNull();
    ...
    private static FakeToolCaller CreateCaller(IReadOnlyList<McpRemoteTool> tools) =>
        new(
            McpProtocolVersions.July2026,
            tools,
            JsonSerializer.SerializeToElement(new WeatherResponse("sunny")));

    private static McpRemoteTool WeatherRemoteTool() =>
        new(new McpToolName("weather.get"), new ToolVersion("2.1"));

    private static Task<WeatherResponse> Forward(WeatherTools tools, WeatherRequest request) =>
        tools.GetAsync(request);
    ...
    private abstract class ValueTaskTools
    {
        ...
        public abstract ValueTask<WeatherResponse> GetAsync(
            WeatherRequest request,
            CancellationToken cancellationToken = default);
    }
    ...
    private sealed class FakeToolCaller(
        McpProtocolVersion protocolVersion,
        IReadOnlyList<McpRemoteTool> tools,
        JsonElement response): IMcpToolCaller
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            return CallHandler?.Invoke(method, request, serializerOptions, cancellationToken) ??
