...
public sealed class DeveloperMessageTests
{
    ...
    [Fact]
    public void Constructor_WhenPartsContainNull_ThrowsArgumentException() => Should.Throw<ArgumentException>(() => CreateDeveloperMessage([null!], ExtensionData.Empty)).ParamName.ShouldBe("parts");

    [Fact]
    public void With_WhenPartsIsDefault_ThrowsArgumentException()
    {
        var message = CreateDeveloperMessage(_sampleParts, ExtensionData.Empty);
