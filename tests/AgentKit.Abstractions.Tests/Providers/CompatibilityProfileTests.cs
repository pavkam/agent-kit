...
/// <summary>Verifies CompatibilityProfile behavior and contracts.</summary>
public sealed class CompatibilityProfileTests
{
    ...
    public void Constructor_WhenMultiplicityIsDefined_PreservesOperationContract(ModelCandidateMultiplicity requestMultiplicity, ModelCandidateMultiplicity responseMultiplicity)
    {
    ...
    private static CompatibilityProfile Profile(CompatibilityProfileKey? key = null, CompatibilityProfileVersion? version = null, ContentHash? fingerprint = null, ModelCandidateMultiplicity requestMultiplicity = ModelCandidateMultiplicity.ExactlyOne, ModelCandidateMultiplicity responseMultiplicity = ModelCandidateMultiplicity.ExactlyOne, ModelUsageReportingMode usageReporting = ModelUsageReportingMode.NotReported, ImmutableArray<JsonSchemaDialectId>? dialects = null, ExtensionData? extensions = null) => new(key ?? new CompatibilityProfileKey("openai-chat"), version ?? new CompatibilityProfileVersion(1), fingerprint ?? new ContentHash("sha256:profile"), requestMultiplicity, responseMultiplicity, usageReporting, dialects ?? [Dialect()], extensions ?? ExtensionData.Empty);
    private static CompatibilityProfile Create(CompatibilityProfileKey key, CompatibilityProfileVersion version, ContentHash fingerprint, ModelCandidateMultiplicity requestMultiplicity, ModelCandidateMultiplicity responseMultiplicity, ModelUsageReportingMode usageReporting, ImmutableArray<JsonSchemaDialectId> dialects, ExtensionData extensions) => new(key, version, fingerprint, requestMultiplicity, responseMultiplicity, usageReporting, dialects, extensions);
