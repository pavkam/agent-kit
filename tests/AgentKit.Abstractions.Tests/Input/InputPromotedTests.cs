...
public sealed class InputPromotedTests
{
    ...
    public void InputPromoted_WhenRecordsDoNotMatchSnapshotOrder_ThrowsArgumentExceptionWithParamName()
    {
        ...
        var exception = ShouldThrowExactly<ArgumentException>(() => new InputPromoted(Snapshot([first.AdmissionId, second.AdmissionId]), [second, first], new SessionVersion(1)));
    ...
    public void InputPromoted_WhenRecordTargetsAnotherLane_ThrowsArgumentExceptionWithParamName()
    {
        ...
        var exception = ShouldThrowExactly<ArgumentException>(() => new InputPromoted(Snapshot([promoted.AdmissionId]), [promoted], new SessionVersion(1)));
        exception.ParamName.ShouldBe("promoted");
    }
