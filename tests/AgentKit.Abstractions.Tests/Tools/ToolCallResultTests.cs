...
/// <summary>Verifies ToolCallResult behavior and contracts.</summary>
public sealed class ToolCallResultTests
{
    ...
    {
        var fixture = Fixture.Create();
        var exception = Should.Throw<ArgumentNullException>(() => fixture.Result(status: ToolTerminalStatus.Succeeded, accepted: false));
    ...
    {
        var fixture = Fixture.Create(effect: ToolEffect.Mutating, idempotency: null);
        var exception = Should.Throw<ArgumentException>(() => fixture.Result(accepted: false, retryable: true, sideEffectCertainty: SideEffectCertainty.Unknown));
        exception.ParamName.ShouldBe("retryable");
    }
    ...
    {
        var fixture = Fixture.Create(effect: ToolEffect.Mutating, idempotency: null);
        var exception = Should.Throw<ArgumentException>(() => fixture.Result(accepted: false, retryable: true, sideEffectCertainty: SideEffectCertainty.PartiallyPerformed));
        exception.ParamName.ShouldBe("retryable");
    }
    ...
    {
        var fixture = Fixture.Create(effect: ToolEffect.Mutating, idempotency: IdempotencyClassification.Idempotent);
        var result = fixture.Result(accepted: false, retryable: true, sideEffectCertainty: SideEffectCertainty.PartiallyPerformed);
    ...
    {
        var fixture = Fixture.Create();
        var exception = Should.Throw<ArgumentOutOfRangeException>(() => fixture.Result(sideEffectCertainty: SideEffectCertainty.NotApplicable));
    ...
    {
        var fixture = Fixture.Create();
        var exception = Should.Throw<ArgumentOutOfRangeException>(() => fixture.Result(sideEffectCertainty: (SideEffectCertainty) 99));
        exception.GetType().ShouldBe(typeof(ArgumentOutOfRangeException));
        exception.ParamName.ShouldBe("sideEffectCertainty");
    }
    ...
    {
        var fixture = Fixture.Create();
        var first = fixture.Result(content: [new ToolResultTextContent("a", TextSemantics.Plain, ExtensionData.Empty), new ToolResultTextContent("b", TextSemantics.Plain, ExtensionData.Empty),]);
        var same = fixture.Result(content: [new ToolResultTextContent("a", TextSemantics.Plain, ExtensionData.Empty), new ToolResultTextContent("b", TextSemantics.Plain, ExtensionData.Empty),]);
        var reordered = fixture.Result(content: [new ToolResultTextContent("b", TextSemantics.Plain, ExtensionData.Empty), new ToolResultTextContent("a", TextSemantics.Plain, ExtensionData.Empty),]);
        first.ShouldBe(same);
