...
/// <summary>Verifies SecurityRequest behavior and contracts.</summary>
public sealed class SecurityRequestTests
{
    [Theory]
    [InlineData(0)]
    [InlineData(1)]
    ...
    public void Constructor_WhenCapturedScopeDiffers_ThrowsArgumentExceptionForAuthorization(int contract)
    {
        ...
        var authorization = Authorization(new SecurityAuthorizationScope(scope.AgentId, scope.SessionId, new BeforeRunOperationCorrelation(new OperationId(Guid.NewGuid()), null)), identity);
    ...
    public void With_WhenCapturedScopeDiffers_ThrowsArgumentExceptionForScope(int contract)
    {
        ...
        {
            0 => () => _ = Request(scope, identity, authorization) with
            {
                Scope = differentScope
            },
            1 => () => _ = Grant(scope, identity, authorization) with
            {
                Scope = differentScope
            },
            _ => () => _ = Enforcement(scope, identity, authorization) with
            {
                Scope = differentScope
            },
        };
    ...
    public void Constructors_WhenCapturedEvidenceMatches_RetainExactAuthorization()
    {
        ...
        (Request(scope, identity, authorization) with
        {
            Scope = scope,
            Identity = identity
        }
