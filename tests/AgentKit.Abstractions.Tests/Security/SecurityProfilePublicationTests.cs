...
/// <summary>Verifies SecurityProfilePublication behavior and contracts.</summary>
public sealed class SecurityProfilePublicationTests
{
    [Fact]
    public void Constructor_WhenPublicationPartIsInvalid_ThrowsWithExactParameterName()
    {
        AssertExact<ArgumentOutOfRangeException>(() => NewPublication(default, DefinitionRevision(), ConfigurationVersion(), ProfileKey(), ProfileVersion(), PolicySnapshot(), AuthorityKey()), "agentId");
        AssertExact<ArgumentOutOfRangeException>(() => NewPublication(AgentId(), DefinitionRevision(), default, ProfileKey(), ProfileVersion(), PolicySnapshot(), AuthorityKey()), "configurationVersion");
        AssertExact<ArgumentNullException>(() => NewPublication(AgentId(), DefinitionRevision(), ConfigurationVersion(), default, ProfileVersion(), PolicySnapshot(), AuthorityKey()), "profileKey");
        AssertExact<ArgumentOutOfRangeException>(() => NewPublication(AgentId(), DefinitionRevision(), ConfigurationVersion(), ProfileKey(), default, PolicySnapshot(), AuthorityKey()), "profileVersion");
        AssertExact<ArgumentNullException>(() => NewPublication(AgentId(), DefinitionRevision(), ConfigurationVersion(), ProfileKey(), ProfileVersion(), null!, AuthorityKey()), "policySnapshot");
        AssertExact<ArgumentNullException>(() => NewPublication(AgentId(), DefinitionRevision(), ConfigurationVersion(), ProfileKey(), ProfileVersion(), PolicySnapshot(), default), "authorityKey");
    }
    ...
    private static SecurityPolicySnapshotReference PolicySnapshot() => new(new SecurityPolicySnapshotId(Guid.Parse("a2222222-2222-2222-2222-222222222222")), new SecurityPolicyVersion(5), new ContentHash("sha256:policy"));
