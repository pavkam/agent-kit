...
/// <summary>Verifies BudgetExecutionCapability behavior and contracts.</summary>
public sealed class BudgetExecutionCapabilityTests
{
    ...
    public void Constructor_WhenBindingMatches_PreservesBorrowedScopeAndEvidence()
    {
        ...
        var capability = new BudgetExecutionCapability(new BudgetProfileKey("standard"), new BudgetProfileVersion(7), identity, correlation, scope);
    ...
    public void Constructor_WhenCorrelationStageMatchesAddress_AcceptsRequiredRunShape(string stage, bool hasRun)
    {
        ...
        var capability = new BudgetExecutionCapability(new BudgetProfileKey("standard"), new BudgetProfileVersion(1), Identity(), correlation, scope);
        capability.Scope.ShouldBeSameAs(scope);
    }
    ...
    public void Constructor_WhenScopeBindingDiffers_ThrowsExactScopeParameter(string mismatch)
    {
        ...
        var address = Address(tenant: mismatch == "tenant" ? "other" : "tenant", principal: mismatch == "principal" ? "other" : "principal", runId: mismatch is "in-run-missing" ? null : mismatch == "in-run-other" ? OtherRunId() : _runId, operationId: mismatch == "operation-missing" ? null : mismatch == "operation-other" ? OtherOperationId() : _operationId);
        var exception = Should.Throw<ArgumentException>(() => new BudgetExecutionCapability(new BudgetProfileKey("standard"), new BudgetProfileVersion(1), identity, correlation, Scope(address)));
    ...
    {
        var exception = Should.Throw<ArgumentOutOfRangeException>(() => new BudgetExecutionCapability(new BudgetProfileKey("standard"), new BudgetProfileVersion(1), Identity(), Correlation("in"), new TestBudgetScope(default, Address(runId: _runId, operationId: _operationId))));
        exception.ParamName.ShouldBe("scope");
    }
    ...
    {
        var exception = Record.Exception(() => new BudgetExecutionCapability(parameter == "profileKey" ? default : new BudgetProfileKey("standard"), parameter == "profileVersion" ? default : new BudgetProfileVersion(1), parameter == "identity" ? null! : Identity(), parameter == "correlation" ? null! : Correlation("in"), parameter == "scope" ? null! : Scope(Address(runId: _runId, operationId: _operationId))));
    ...
    {
        var exception = Should.Throw<ArgumentNullException>(() => new BudgetExecutionCapability(new BudgetProfileKey("standard"), new BudgetProfileVersion(1), Identity(), Correlation("in"), new NullAddressBudgetScope()));
        exception.ParamName.ShouldBe("scope");
    }
    ...
    public void ThrowIfInvalidBudgetExecutionBinding_WhenReferenceIsNull_ThrowsInferredParameterNames()
    {
        ...
        Should.Throw<ArgumentNullException>(() => ArgumentException.ThrowIfInvalidBudgetExecutionBinding(address, Identity(), Correlation("in"))).ParamName.ShouldBe("address");
        Should.Throw<ArgumentNullException>(() => ArgumentException.ThrowIfInvalidBudgetExecutionBinding(Address(runId: _runId, operationId: _operationId), identity, Correlation("in"))).ParamName.ShouldBe("identity");
        Should.Throw<ArgumentNullException>(() => ArgumentException.ThrowIfInvalidBudgetExecutionBinding(Address(runId: _runId, operationId: _operationId), Identity(), correlation)).ParamName.ShouldBe("correlation");
    }

    [Fact]
    public void ThrowIfInvalidBudgetExecutionBinding_WhenBindingDiffers_UsesExplicitParameterName()
    {
        var exception = Should.Throw<ArgumentException>(() => ArgumentException.ThrowIfInvalidBudgetExecutionBinding(Address(tenant: "other", runId: _runId, operationId: _operationId), Identity(), Correlation("in"), "binding"));
