...
/// <summary>Verifies CompactionRecord behavior and contracts.</summary>
public sealed class CompactionRecordTests
{
    [Fact]
    public void CompactionRecord_Constructor_WhenActiveWithoutCheckpoint_ThrowsArgumentException()
    {
        var exception = Should.Throw<ArgumentException>(() => new CompactionRecord(Context(), new SessionVersion(1), null, CompactionRecordStatus.Active, Manifest(), null, null, null, DateTimeOffset.UnixEpoch, ExtensionData.Empty));
    ...
    {
        var exception = Should.Throw<ArgumentException>(() => new CompactionRecord(Context(), new SessionVersion(1), null, CompactionRecordStatus.Rejected, Manifest(), null, null, null, DateTimeOffset.UnixEpoch, ExtensionData.Empty));
        exception.ParamName.ShouldBe("status");
    }

    [Fact]
    public void CompactionRecord_Equality_WhenSameValues_InstancesAreEqual() => Record().ShouldBe(Record());
