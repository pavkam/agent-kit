...
/// <summary>Verifies CompactionRetryContinuationCause behavior and contracts.</summary>
public sealed class CompactionRetryContinuationCauseTests
{
    ...
    {
        var exception = Should.Throw<ArgumentException>(() => new CompactionRetryContinuationCause(_requestId, Compaction() with { Record = null! }));
    ...
    private static CompactionSucceeded WithMalformedPart(CompactionSucceeded compaction, string part) => part switch
    {
        "resultContext" => compaction with
        {
            Context = null!
        },
        "recordContext" => compaction with
        {
            Record = compaction.Record with
            {
                Context = null!
    ...
    private static CompactionSucceeded Compaction()
    {
        ...
        var manifest = new CompactionManifest(new CompactionManifestId(Guid.Parse("bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb")), context, _branchId, new SessionVersion(3), new CompactionSourceRange(new SessionSequence(1), new SessionSequence(5)), new SessionSequence(6), new CompactionProducer(new CompactionStrategyKey("test"), deterministic: true, ExtensionData.Empty), new ContextEpoch(1), new CompactionSizeEstimate(20, 100, 1), new CompactionSizeEstimate(5, 25, 1), DateTimeOffset.UnixEpoch, ExtensionData.Empty);
