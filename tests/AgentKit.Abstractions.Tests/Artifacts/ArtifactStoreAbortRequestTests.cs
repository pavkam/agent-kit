...
/// <summary>Verifies ArtifactStoreAbortRequest behavior and contracts.</summary>
public sealed class ArtifactStoreAbortRequestTests
{
    ...
    public void Constructor_WhenCalledWithValidArguments_InitializesProperties()
    {
        ...
        var request = new ArtifactStoreAbortRequest(PreparationId(), ArtifactAbortReason.Cancelled, scope, identity, grant, key);
    ...
    {
        var identity = Identity();
        var exception = Should.Throw<ArgumentOutOfRangeException>(() => new ArtifactStoreAbortRequest(default, ArtifactAbortReason.Cancelled, Scope(), identity, Grant(identity), new IdempotencyKey("abort")));
    ...
    {
        var identity = Identity();
        var exception = Should.Throw<ArgumentOutOfRangeException>(() => new ArtifactStoreAbortRequest(PreparationId(), (ArtifactAbortReason) 999, Scope(), identity, Grant(identity), new IdempotencyKey("abort")));
    ...
    {
        var identity = Identity();
        var exception = Should.Throw<ArgumentNullException>(() => new ArtifactStoreAbortRequest(PreparationId(), ArtifactAbortReason.Cancelled, null!, identity, Grant(identity), new IdempotencyKey("abort")));
    ...
    {
        var identity = Identity();
        var exception = Should.Throw<ArgumentNullException>(() => new ArtifactStoreAbortRequest(PreparationId(), ArtifactAbortReason.Cancelled, Scope(), null!, Grant(identity), new IdempotencyKey("abort")));
    ...
    {
        var identity = Identity();
        var exception = Should.Throw<ArgumentNullException>(() => new ArtifactStoreAbortRequest(PreparationId(), ArtifactAbortReason.Cancelled, Scope(), identity, null!, new IdempotencyKey("abort")));
    ...
    {
        var identity = Identity();
        var exception = Should.Throw<ArgumentException>(() => new ArtifactStoreAbortRequest(PreparationId(), ArtifactAbortReason.Cancelled, Scope(), identity, Grant(identity), default));
    ...
    private static SecurityGrant Grant(ExecutionIdentity identity) => new(new GrantId(Guid.Parse("90000000-0000-0000-0000-000000000009")), new SecurityRequestId(Guid.Parse("a0000000-0000-0000-0000-00000000000a")), Scope(), identity, new ComponentId("artifact-store"), SecurityOperationKind.Artifact, SecurityEffect.Delete, [ArtifactSecurityBinding.PreparationResource(PreparationId())], new InputFingerprint("fingerprint"), new SecurityPolicyVersion(1), new SecurityRevocationVersion(1), DateTimeOffset.UnixEpoch, DateTimeOffset.UnixEpoch.AddMinutes(1), 1);
    ...
    {
        var identity = Identity();
        var original = new ArtifactStoreAbortRequest(PreparationId(), ArtifactAbortReason.Cancelled, Scope(), identity, Grant(identity), new IdempotencyKey("abort"));
