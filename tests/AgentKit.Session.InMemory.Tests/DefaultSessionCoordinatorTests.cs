...
/// <summary>Verifies DefaultSessionCoordinator behavior and contracts.</summary>
public sealed class DefaultSessionCoordinatorTests
{
    ...
    public async Task CreateAsync_WithRealGrantStoreDirectoryAndStore_ConsumesIndependentExactGrants()
    {
        ...
        _ = services.AddSingleton<ISecurityAuthoritySelector>(provider => new IntegrationAuthoritySelector(new RegisteringAuthority(provider.GetRequiredService<ISecurityGrantStore>(), timeProvider)));
        ...
        var result = await coordinator.CreateAsync(request, TestSecurityEvidence.SessionProfile("agentkit.in-memory"), TestContext.Current.CancellationToken);
    ...
    {
        public ValueTask<SecurityAuthorizationCaptureResult> SelectAsync(SecurityAuthorizationCaptureRequest request, CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            return ValueTask.FromResult<SecurityAuthorizationCaptureResult>(new SecurityAuthorizationCaptured(new SecurityAuthorizationContext(request.ProfileKey, new SecurityProfileVersion(1), new SecurityPolicySnapshotReference(new SecurityPolicySnapshotId(Guid.Parse("22222222-2222-2222-2222-222222222222")), new SecurityPolicyVersion(1), new ContentHash("sha256:policy")), new ComponentKey<ISecurityAuthority>("authority"), request.AgentDefinitionRevision, request.ConfigurationVersion, request.Scope, request.Identity)));
        }
    }
    ...
    private sealed class IntegrationAuthoritySelector(ISecurityAuthority authority): ISecurityAuthoritySelector
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return ValueTask.FromResult<SecurityAuthoritySelectionResult>(new SecurityAuthoritySelected(authorization, authority));
        }
    }

    private sealed class RegisteringAuthority(ISecurityGrantStore grantStore, TimeProvider timeProvider): ISecurityAuthority
    {
        ...
        {
            var authorization = request.Authorization ?? throw new InvalidOperationException("Coordinator requests must retain captured authorization.");
            var grant = new SecurityGrant(new GrantId(Guid.NewGuid()), request.Id, request.Scope, request.Identity, authorization, request.Audience, request.Kind, request.Effect, request.Resources, request.InputFingerprint, authorization.PolicySnapshot.Version, new SecurityRevocationVersion(1), timeProvider.GetUtcNow(), request.Deadline, allowedUses: 1);
