...
using AgentKit.Providers.DeepSeek.Tests.Fakes;

/// <summary>Verifies DeepSeekLlmModel behavior and contracts.</summary>
public sealed class DeepSeekLlmModelTests
{
    ...
    private static ModelDescriptor CreateDescriptor() => new(new ModelAlias("chat"), DeepSeekProviderDefaults.ProviderId, DeepSeekProviderDefaults.ApiFamily, new ModelId("deepseek-chat"), deploymentId: null, DeepSeekProviderDefaults.DefaultCapabilities, DeepSeekProviderDefaults.DefaultLimits, pricing: null, ExtensionData.Empty);
    [Fact]
    public async Task ExecuteAsync_WhenUsingApiKeyCredential_SendsBearerHeaderAndReturnsCompletedResponse()
    {
        ...
        completed.Response.Parts[0].ShouldBeOfType<TextPart>().Text.ShouldBe("Hello from DeepSeek!");
        completed.Response.Identity.ProviderId.ShouldBe(DeepSeekProviderDefaults.ProviderId);
        _ = handler.Requests.ShouldHaveSingleItem();
        handler.Requests[0].RequestUri.ShouldBe(new Uri("https://api.deepseek.com/chat/completions"));
        handler.Requests[0].Headers.Authorization!.Scheme.ShouldBe("Bearer");
        handler.Requests[0].Headers.Authorization!.Parameter.ShouldBe("ds-real-looking-key");
    }
    ...
    public async Task ExecuteAsync_WhenUsingExpiredOAuthCredential_FailsAuthenticationWithoutSendingHttpRequest()
    {
        ...
        var model = new DeepSeekLlmModel(descriptor, DeepSeekProviderDefaults.CreateProfile(options), new OpenAIRequestTranslator(), new OpenAIChatCompletionResponseParser(new SequentialToolCallIdGenerator()), new DelegatingOAuthCredentialSource(new StaticOAuthTokenProvider(expiredToken)), new HttpClient(handler), new FakeTimeProvider(Now));
        var observer = new RecordingModelResponseObserver();
        var result = await model.ExecuteAsync(CreateRequest(descriptor), observer, TestContext.Current.CancellationToken);
