...
/// <summary>Verifies OllamaLlmModel behavior and contracts.</summary>
public sealed class OllamaLlmModelTests
{
    ...
    private static ModelDescriptor CreateDescriptor() => new(new ModelAlias("chat"), OllamaProviderDefaults.ProviderId, OllamaProviderDefaults.ApiFamily, new ModelId("llama3.3"), deploymentId: null, OllamaProviderDefaults.DefaultCapabilities, OllamaProviderDefaults.DefaultLimits, pricing: null, ExtensionData.Empty);
    [Fact]
    public async Task ExecuteAsync_WhenUsingApiKeyCredential_SendsBearerHeaderAndReturnsCompletedResponse()
    {
        ...
        var model = new OllamaLlmModel(descriptor, OllamaProviderDefaults.CreateProfile(options), new OpenAIRequestTranslator(), new OpenAIChatCompletionResponseParser(new SequentialToolCallIdGenerator()), new StaticApiKeyCredentialSource("real-looking-key"), new HttpClient(handler), new FakeTimeProvider(Now));
    ...
    public async Task ExecuteAsync_WhenUsingExpiredOAuthCredential_FailsAuthenticationWithoutSendingHttpRequest()
    {
        ...
        var model = new OllamaLlmModel(descriptor, OllamaProviderDefaults.CreateProfile(options), new OpenAIRequestTranslator(), new OpenAIChatCompletionResponseParser(new SequentialToolCallIdGenerator()), new DelegatingOAuthCredentialSource(new StaticOAuthTokenProvider(expiredToken)), new HttpClient(handler), new FakeTimeProvider(Now));
        var observer = new RecordingModelResponseObserver();
        var result = await model.ExecuteAsync(CreateRequest(descriptor), observer, TestContext.Current.CancellationToken);
