...
/// <summary>Verifies ledger diagnostics are truthful, bounded, content-free, and failure-isolated.</summary>
public sealed class BudgetLedgerDiagnosticsTests
{
    ...
    public async Task CreateScopeAsync_WhenAddressHasSessionAndRun_EmitsApplicableCorrelation()
    {
        ...
        var created = (await ledger.CreateScopeAsync(ScopeRequest("hierarchy-correlation", address), TestContext.Current.CancellationToken))
            .ShouldBeOfType<BudgetLedgerScopeCreated>();
    ...
    public async Task Operations_WhenInvalidCancelledOrFaulted_RespectDiagnosticBoundary()
    {
        ...
        source.Cancel();
        _ = await Should.ThrowAsync<OperationCanceledException>(async () => await ledger.GetSnapshotAsync(
            new BudgetLedgerScopeReference(new BudgetScopeId(Guid.NewGuid()), Address()), source.Token));
        _ = await Should.ThrowAsync<BudgetLedgerReferenceUnavailableException>(async () => await ledger.GetSnapshotAsync(
            new BudgetLedgerScopeReference(new BudgetScopeId(Guid.NewGuid()), Address()), TestContext.Current.CancellationToken));
    ...
    public async Task MarkStartedAsync_WhenReservationExpired_EmitsExpiredOutcomeWithoutLimitFailure()
    {
        ...
        var activity = activities.Single(item =>
            ...
            && Equals(item.GetTagItem(AgentKitTagNames.BudgetOperation), "mark_started"));
        ...
        var entry = logger.Entries.Single(item =>
            item.Tags.TryGetValue("BudgetOperation", out var operation)
            && Equals(operation, "mark_started"));
    ...
    private static bool IsLedgerInstrument(Instrument instrument) =>
        instrument.Meter.Name == AgentKitDiagnostics.MeterName
        && instrument.Name is AgentKitMetricNames.BudgetLedgerOperationCount or AgentKitMetricNames.BudgetLedgerOperationDuration;

    private static void RecordForTrace(
        ActivityTraceId traceId,
        List<(string Name, IReadOnlyDictionary<string, object?> Tags)> measurements,
    ...
    private static async Task<BudgetLedgerReservationReference> ReserveAsync(IBudgetLedger ledger, BudgetLedgerScopeReference scope, string key) =>
        (await ledger.ReserveBatchAsync(new BudgetLedgerBatchReserveRequest(scope,
            [new BudgetReservationRequest(scope.Id, new BudgetDimension("test.sum"), 1, new BudgetUnit("count"), new OperationId(Guid.Parse("20000000-0000-0000-0000-000000000001")), null, new IdempotencyKey(key))]), TestContext.Current.CancellationToken))
    ...
    private sealed class RecordingLogger: ILogger<InMemoryBudgetLedger>
    {
        ...
        {
            var tags = state is IEnumerable<KeyValuePair<string, object?>> values
