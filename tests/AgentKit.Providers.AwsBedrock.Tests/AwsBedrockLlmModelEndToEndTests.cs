...
/// <summary>
/// End-to-end tests for <see cref="AwsBedrockLlmModel"/>, exercising the
/// full pipeline (capability check, credential resolution, translation,
/// AWS Signature Version 4 signing, transport, and response parsing)
/// against a stub HTTP handler serving fixture payloads. No test in this
/// class performs a real network call.
/// </summary>
public sealed class AwsBedrockLlmModelEndToEndTests
{
    ...
    private static AwsBedrockLlmModel CreateModel(
        HttpMessageHandler handler,
        IAwsCredentialSource credentials,
        ModelDescriptor? descriptor = null,
        TimeProvider? timeProvider = null,
        AwsBedrockProviderOptions? options = null) =>
        new(
            descriptor ?? TestModels.ClaudeSonnet,
            options ?? new AwsBedrockProviderOptions { Region = "us-east-1" },
            new AwsBedrockRequestTranslator(),
            new AwsBedrockResponseParser(new SequentialToolCallIdGenerator()),
            credentials,
            new HttpClient(handler),
            timeProvider ?? new FakeTimeProvider(Now));

    private static StaticAwsCredentialSource CreateCredentials() =>
        new(new AwsSigV4Credential("AKIAIOSFODNN7EXAMPLE", "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY", null));
    ...
    public async Task ExecuteAsync_WhenSessionTokenCredential_IncludesSecurityTokenHeader()
    {
        var handler = StubHttpMessageHandler.FromFixture(HttpStatusCode.OK, "responses/buffered_text.json");
        var options = new AwsBedrockProviderOptions { Region = "us-east-1", PreferStreaming = false };
        var credentials = new StaticAwsCredentialSource(
            new AwsSigV4Credential("AKIAIOSFODNN7EXAMPLE", "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY", "session-token-value"));
    ...
    public async Task ExecuteAsync_WhenStreamingSuccess_ReturnsCompletedResponseFromConverseStreamPath()
    {
        var handler = StubHttpMessageHandler.FromFixture(
            HttpStatusCode.OK, "responses/streaming_text.bin", "application/vnd.amazon.eventstream");
    ...
    public async Task ExecuteAsync_WhenAccessDenied_ReturnsAuthorizationFailureUsingErrorTypeHeader()
    {
        var handler = StubHttpMessageHandler.FromFixture(
            ...
            "responses/error_access_denied.json",
            configureHeaders: response => response.Headers.Add("x-amzn-errortype", "AccessDeniedException"));
        var options = new AwsBedrockProviderOptions { Region = "us-east-1", PreferStreaming = false };
        var model = CreateModel(handler, CreateCredentials(), options: options);
        var request = CreateRequest(TestModels.ClaudeSonnet, Now.AddMinutes(1));
        var observer = new RecordingModelResponseObserver();
    ...
    public async Task ExecuteAsync_WhenThrottled_ReturnsThrottlingFailure()
    {
        var handler = StubHttpMessageHandler.FromFixture(
            ...
            configureHeaders: response => response.Headers.Add("x-amzn-errortype", "ThrottlingException"));
        var options = new AwsBedrockProviderOptions { Region = "us-east-1", PreferStreaming = false };
        var model = CreateModel(handler, CreateCredentials(), options: options);
        var request = CreateRequest(TestModels.ClaudeSonnet, Now.AddMinutes(1));
        var observer = new RecordingModelResponseObserver();
    ...
    public async Task ExecuteAsync_WhenProviderRequestOptionsCarryExtensionData_ForwardsThemInSentBody()
    {
        ...
        var providerOptions = new ProviderRequestOptions(
            new ExtensionData(ImmutableDictionary<string, ExtensionValue>.Empty.Add("performanceConfig", extensionValue)));
