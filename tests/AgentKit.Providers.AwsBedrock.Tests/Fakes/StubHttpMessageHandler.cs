...
/// receives (including its body, read eagerly before disposal), so tests
...
internal sealed class StubHttpMessageHandler: HttpMessageHandler
{
    ...
    /// <summary>
    /// Gets the request body bytes captured for each received request, in
    ...
    /// <param name="mediaType">The response content media type.</param>
    /// <param name="configureHeaders">An optional callback that adds additional response headers.</param>
    /// <returns>A configured <see cref="StubHttpMessageHandler"/>.</returns>
    public static StubHttpMessageHandler FromFixture(
        HttpStatusCode statusCode,
        string fixtureRelativePath,
        string mediaType = "application/json",
        Action<HttpResponseMessage>? configureHeaders = null) =>
        new(_ =>
        {
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        RequestBodies.Add(
            request.Content is null ? null : await request.Content.ReadAsByteArrayAsync(cancellationToken).ConfigureAwait(false));
