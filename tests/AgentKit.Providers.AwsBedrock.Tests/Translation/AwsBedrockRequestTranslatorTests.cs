...
public sealed class AwsBedrockRequestTranslatorTests
{
    ...
    public void Translate_WhenToolResultOutcomeIsFailure_SerializesErrorStatus()
    {
        ...
        var toolMessage = TestMessages.Tool(
            new ToolResultPart(callId, toolReference, new ToolCallOutcome(ToolCallOutcomeKind.Failed, ToolTerminalStatus.InvocationFailed, SideEffectCertainty.Unknown, false, "boom", ExtensionData.Empty), [new TextPart("failed", TextSemantics.Plain, ExtensionData.Empty)], new ToolResultProjectionInfo(ToolResultProjectionPolicyReference.Default, [], 0, 0), ExtensionData.Empty));
