...
public sealed class OperationAcceptedSessionEntryCodecTests
{
    [Fact]
    ...
    public void Decode_WhenUnknownNestedEvidenceIsWithinBounds_PreservesExactWire()
    {
        ...
        var json = Encoding.UTF8.GetString(encoded.Wire.Payload.AsSpan())
            .Replace("\"method\":", "\"future\":{\"x\":1},\"method\":", StringComparison.Ordinal);
    ...
    {
        var malformed = OperationAcceptedSessionEntryCodecTestData.Entry() with { Id = default };
    ...
    public void Encode_WhenRetainedConfigurationTextIsMalformed_RejectsWithoutReplacement()
    {
        ...
        var malformedConfiguration = OperationAcceptedSessionEntryCodecTestData.Entry(
    ...
    public void Encode_WhenSuccessful_EmitsValidatedAcceptedStateCorrelation()
    {
        ...
        using var listener = new ActivityListener
        {
            ...
            {
                if (activity.ParentSpanId == parent.SpanId && activity.TraceId == parent.TraceId
                    && Equals(activity.GetTagItem(AgentKitTagNames.SessionEntryCodecOperation), "operation-accepted.encode"))
                {
                    stopped = activity;
                }
            },
    ...
    {
        var entry = OperationAcceptedSessionEntryCodecTestData.Entry();
        var sufficient = new OperationAcceptedSessionEntryCodec(TimeProvider.System,
            NullLogger<OperationAcceptedSessionEntryCodec>.Instance, new SessionEntryCodecLimits(1_048_576, 64, 65_536, 24));
        var shallow = new OperationAcceptedSessionEntryCodec(TimeProvider.System,
            NullLogger<OperationAcceptedSessionEntryCodec>.Instance, new SessionEntryCodecLimits(1_048_576, 64, 65_536, 2));
    ...
    private static OperationAcceptedSessionEntryCodec Codec() =>
        new(TimeProvider.System, NullLogger<OperationAcceptedSessionEntryCodec>.Instance);
