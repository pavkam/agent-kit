...
public sealed class PortableSessionEntryCodecTests
{
    ...
    public void Constructor_WhenLimitsDiffer_CapturesIndependentDescriptorAndRuntimePolicy()
    {
        ...
        var exact = new ExecutionLaneProvisionedSessionEntryCodec(TimeProvider.System,
            NullLogger<ExecutionLaneProvisionedSessionEntryCodec>.Instance, exactLimits);
        var smaller = new ExecutionLaneProvisionedSessionEntryCodec(TimeProvider.System,
            NullLogger<ExecutionLaneProvisionedSessionEntryCodec>.Instance, smallerLimits);
    ...
    {
        var shallow = new ExecutionLaneProvisionedSessionEntryCodec(TimeProvider.System,
            NullLogger<ExecutionLaneProvisionedSessionEntryCodec>.Instance, Limits(maximumJsonDepth: 1));
        var sufficient = new ExecutionLaneProvisionedSessionEntryCodec(TimeProvider.System,
            NullLogger<ExecutionLaneProvisionedSessionEntryCodec>.Instance, Limits(maximumJsonDepth: 2));
    ...
    {
        var shallow = new InputPromotedSessionEntryCodec(TimeProvider.System,
            NullLogger<InputPromotedSessionEntryCodec>.Instance, Limits(maximumJsonDepth: 1));
        var sufficient = new InputPromotedSessionEntryCodec(TimeProvider.System,
            NullLogger<InputPromotedSessionEntryCodec>.Instance, Limits(maximumJsonDepth: 2));
    ...
    public void Constructor_WhenLimitsIsNull_ThrowsBeforeAssignment()
    {
        var exception = Should.Throw<ArgumentNullException>(() => new InputPromotedSessionEntryCodec(
            TimeProvider.System, NullLogger<InputPromotedSessionEntryCodec>.Instance, null!));
    ...
    {
        Should.Throw<ArgumentNullException>(() => PortableSessionEntryJson.WriteGuid(null!, "id", Id(1)))
            .ParamName.ShouldBe("writer");
        using var writer = new Utf8JsonWriter(new ArrayBufferWriter<byte>());

        Should.Throw<ArgumentOutOfRangeException>(() => PortableSessionEntryJson.WriteGuid(writer, "id", default))
            .ParamName.ShouldBe("value");
        Should.Throw<ArgumentOutOfRangeException>(() => PortableSessionEntryJson.WriteOptionalGuid(writer, "id", Guid.Empty))
            .ParamName.ShouldBe("value");
    }
    ...
    public void ExecutionLaneCodec_WhenEncodingKnownEntry_WritesStableGoldenWire()
    {
        ...
        Encoding.UTF8.GetString(encoded.Wire.Payload.AsSpan()).ShouldBe(
            /*lang=json,strict*/
    ...
    public void ExecutionLaneCodec_WhenUnknownNestedFieldIsWithinBounds_DecodesAndRetainsOriginalWire()
    {
        ...
        var json = Encoding.UTF8.GetString(encoded.Wire.Payload.AsSpan())
            .Replace("\"sessionId\":", "\"future\":{\"nested\":[1,2]},\"sessionId\":", StringComparison.Ordinal);
    ...
    public void ExecutionLaneCodec_WhenUnknownNestedObjectHasDuplicateProperty_RejectsBeforeInterpretation()
    {
        ...
        var encoded = codec.Encode(LaneEntry()).ShouldBeOfType<SessionEntryEncoded>();
        var json = Encoding.UTF8.GetString(encoded.Wire.Payload.AsSpan())
            .Replace("\"sessionId\":", "\"future\":{\"x\":1,\"x\":2},\"sessionId\":", StringComparison.Ordinal);
    ...
    {
        var codec = LaneCodec();
        var baseline = codec.Encode(LaneEntry(configurationFingerprint: "a"))
            .ShouldBeOfType<SessionEntryEncoded>().Wire.Payload.Length;
        var exactTextLength = codec.Descriptor.Limits.MaximumPayloadBytes - baseline + 1;

        var below = codec.Encode(LaneEntry(configurationFingerprint: new string('a', exactTextLength - 1)))
            .ShouldBeOfType<SessionEntryEncoded>();
        var exact = codec.Encode(LaneEntry(configurationFingerprint: new string('a', exactTextLength)))
            .ShouldBeOfType<SessionEntryEncoded>();
    ...
    public void InputPromotedCodec_WhenAdmissionArrayCannotFitPayload_RejectsBeforeDistinctEncodingWork()
    {
        ...
        var entry = new InputPromotedSessionEntry(template.Id, template.Address,
            (InRunOperationCorrelation) template.Correlation, template.BranchId, template.Sequence,
            template.CausalParentId, template.RecordedAt, template.SchemaVersion, template.ExecutionLaneId,
            admissions[0], template.Cutoff, admissions);
    ...
    public void ExecutionLaneCodec_WhenJsonContainsLoneSurrogateEscape_Rejects(
        string original,
        string replacement)
    {
        var codec = LaneCodec();
    ...
    public void ExecutionLaneCodec_WhenCompatibleFieldContainsEscapedSurrogatePair_DecodesAndRetainsWire()
    {
        ...
        var encoded = codec.Encode(LaneEntry()).ShouldBeOfType<SessionEntryEncoded>();
        var json = Encoding.UTF8.GetString(encoded.Wire.Payload.AsSpan())
            .Replace("\"sessionId\":", "\"future\":\"\\uD83D\\uDE00\",\"sessionId\":", StringComparison.Ordinal);
    ...
    {
        var entry = LaneEntry() with { Address = null!, Id = default };
    ...
    public void Encode_WhenDiagnosticsThrow_PreservesResultAndAmbientParent()
    {
        ...
        var codec = new ExecutionLaneProvisionedSessionEntryCodec(
            new ThrowingTimeProvider(), new ThrowingLogger<ExecutionLaneProvisionedSessionEntryCodec>());
    ...
    public void Encode_WhenSuccessful_EmitsCorrelatedContentFreeActivityAndLog()
    {
        ...
        using var listener = new ActivityListener
        {
            ...
            {
                if (Equals(activity.GetTagItem(AgentKitTagNames.SessionEntryCodecOperation),
                    "execution-lane-provisioned.encode")
                    && activity.ParentSpanId == parent.SpanId
    ...
    public void Observation_WhenSemanticActionFaults_PreservesExceptionAndLogsOnlyNormalizedType()
    {
        ...
        var actual = Should.Throw<InvalidOperationException>(() => SessionEntryCodecObservation.Observe<SessionEntryEncodeResult>(
            TimeProvider.System, logger, "execution-lane-provisioned.encode", LaneEntry(),
            () => throw expected));
    ...
    public void Encode_WhenMeasured_UsesOnlyBoundedMetricTags()
    {
        ...
        using var listener = new MeterListener
        {
            InstrumentPublished = (instrument, meterListener) =>
            {
                if (instrument.Meter.Name == AgentKitDiagnostics.MeterName
                    && instrument.Name == AgentKitMetricNames.SessionEntryCodecCount)
                {
        ...
        listener.SetMeasurementEventCallback<long>((_, _, tags, _) =>
        {
            ...
            {
                matches |= tag.Key == AgentKitTagNames.SessionEntryCodecOperation
                    && Equals(tag.Value, "execution-lane-provisioned.encode");
            }

            var current = Activity.Current;
            if (matches && current?.TraceId == parent.TraceId
                && (current.SpanId == parent.SpanId || current.ParentSpanId == parent.SpanId))
            {
    ...
    private static SessionEntryCodecLimits Limits(int maximumJsonDepth) =>
        new(1_048_576, 64, 65_536, maximumJsonDepth);
    ...
    private static string AddUnknownObject(string valid, int childCount) =>
        valid[..^1] + ",\"future\":{"
        + string.Join(',', Enumerable.Range(0, childCount).Select(index => $"\"x{index}\":0")) + "}}";

    private static ExecutionLaneProvisionedSessionEntry LaneEntry(
        string profileKey = "default",
        string configurationFingerprint = "sha256:test") => new(
        new SessionEntryId(Id(1)), new SessionAddress(new AgentId(Id(2)), new SessionId(Id(3))),
        new BeforeRunOperationCorrelation(new OperationId(Id(4)), null), new BranchId(Id(5)), new SessionSequence(1),
        null, DateTimeOffset.UnixEpoch, Version, new ExecutionLaneId(Id(6)), new SessionLaneRevision(1),
        new SessionProfileReference(new SessionProfileKey(profileKey), new SessionProfileVersion(1)),
        new RunConfigurationReference(new ConfigurationVersion(1), new RunPolicyVersion(1), new ContentHash(configurationFingerprint)));

    private static InputPromotedSessionEntry PromotionEntry() => new(
        new SessionEntryId(Id(11)), new SessionAddress(new AgentId(Id(2)), new SessionId(Id(3))),
        new InRunOperationCorrelation(new OperationId(Id(4)), new RunId(Id(7)), new TurnId(Id(8))),
        new BranchId(Id(5)), new SessionSequence(2), new SessionEntryId(Id(1)), DateTimeOffset.UnixEpoch, Version,
        new ExecutionLaneId(Id(6)), new AdmissionId(Id(9)), new SessionSequence(1), [new AdmissionId(Id(9)), new AdmissionId(Id(10))]);
