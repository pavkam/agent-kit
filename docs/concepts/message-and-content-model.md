...
known size is never negative. Reasoning content enforces its visibility contract
the same way: redacted and signature-only reasoning carry no text, and
...
these are cross-field invariants, the governed members are read-only rather than
`init`, so a `with` expression cannot break the agreement.
