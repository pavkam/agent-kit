...
| --- | --------------------------------------------------------------------- | ---------- | ------ | ------------------- |
| 1   | [Run envelope and admission](run-envelope-and-admission.md)           | –          | 14     | prerequisites 1,3–5 |
| 2   | [Hook kernel](hook-kernel.md)                                         | –          | 13     | –                   |
| 3   | [Permissions, approvals, audit](permissions-approvals-and-audit.md)   | 2          | 15     | –                   |
...
- `HookDispatchContext` is defined per dispatch (`extensions.md:102-105`) but
  ...
  `HookActivationScope` that creates per-dispatch contexts.
...
- Loop-level `AgentKit.AgentRunRequest` (Abstractions) collides with the facade
  `AgentRunRequest` in `composition-and-configuration.md:277-284`; one must be
  renamed before WS1-C8.
- `RunLimitFailure` requires a `BudgetLimitFailure`, but a turn limit has no
  budget scope; WS1-C7 needs either a synthetic builder or a second constructor.
