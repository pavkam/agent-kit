...
| `ITool` implementations                                                                                                                                                                                                                                                                                                                                     | 18 across 16 packages              | all `sealed`, all take unkeyed `ISecurityAuthority`; `src/AgentKit.Tools.FileSystem/` is an empty directory                                                                                                             |
...
| `ToolRegistrationCatalog`, `ToolCatalogDiscovery`, `ToolDiscoveryCapture`, `ToolCatalogMerger`, `ToolCatalogCapture`, `StaticToolProvider`, `RejectingToolCatalogMergePolicy`, `AddToolset/AddToolProvider/AddStaticToolProvider/...`                                                                                                                       | EXISTS-UNWIRED                     | no coordinator chains them; `TransferToCatalog` and `MergeAsync` never called in production; `tools.md:471` admits it                                                                                                   |
...
| `IToolExecutor`, `IToolScheduler`, `ToolBatch(Result)`, `ToolBatchFailureMode`, `UnknownSchedulingMode`, `ToolExecutionCapability`, `ToolInvocationContext`, `IToolProgressReporter` (C1 contracts) | EXISTS-UNWIRED | `Abstractions/Tools/IToolExecutor.cs`, `IToolScheduler.cs`, `ToolBatch.cs`, `ToolBatchEntry.cs`, `ToolBatchResult.cs`, `ToolExecutionCapability.cs`, `ToolInvocationContext.cs`, `IToolProgressReporter.cs`; no implementation or caller yet (C2, C5a-c) |
| `IToolCallRecorder`, `IToolResultProjector`, `IToolEventSink`, `IToolResolver`, `IToolArgumentValidator`, `IToolExecutionPolicy(+Selector)`, `IToolResultNormalizer`, `ValidatedToolCall`, `PreparedToolCall`, `ResolvedToolCall`, `ToolRuntimeOptions`, `ToolRetryPolicy` | MISSING                            | –                                                                                                                                                                                                                       |
