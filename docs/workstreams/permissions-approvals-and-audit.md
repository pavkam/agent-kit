...
- [x] WS3-C7a typed `RevokeAsync` and revocation generation
- [x] WS3-C7b grant-lifecycle audit in grant stores
- [x] WS3-C7c flip `RevokeAsync` to abstract
- [x] WS3-C8a approval handler dispatcher
- [x] WS3-C8b `ResolveAsync`, durable deferral, `SecurityApprovalRequired`
- [x] WS3-C8c responder binding and authentication evidence
- [x] WS3-C9 SQLite approval store
- [x] WS3-C10a/b/c tools and artifacts move to the selector
...
| durable deferral                                                                                                                                                                                                                                                                                                                            | WIRED (WS3-C8b)         | `ApprovalBrokerDeferred`; authority emits `SecurityApprovalRequired`                                                                                                                                                              |
| approver identity bound into grants                                                                                                                                                                                                                                                                                                         | WIRED (WS3-C8c)         | Optional `SecurityGrant.Approval` (`ApprovalResponseId`)                                                                                                                                                                          |
| constraint intersection                                                                                                                                                                                                                                                                                                                     | WIRED (WS3-C3)          | `SecurityAllowConstraintsAlgebra` and `SecurityPolicyResult.Constraints` in `AuthorizeCoreAsync`                                                                                                                                  |
| revocation generation                                                                                                                                                                                                                                                                                                                       | WIRED (WS3-C7a)         | `ISecurityRevocationGeneration`; live version in `SecurityAuthority`                                                                                                                                                              |
| audit coverage                                                                                                                                                                                                                                                                                                                              | WIRED (WS3-C5/C7b)      | Authority/broker `Request`/`Decision`/`Approval`; grant stores emit `GrantLifecycle` when configured                                                                                                                              |
| `ISecurityAuthoritySelector` consumers                                                                                                                                                                                                                                                                                                      | WIRED (WS3-C10)         | Session, artifacts, tools, and `AgentKit.Context.Project` select before `AuthorizeAsync`                                                                                                                                          |
| bootstrap capability                                                                                                                                                                                                                                                                                                                        | WIRED (WS3-C11)         | `SecurityControlPlaneBootstrap`; durable store `InitializeAsync` gate                                                                                                                                                             |
| typed `ISecurityGrantStore.RevokeAsync`                                                                                                                                                                                                                                                                                                     | WIRED (WS3-C7c)         | `RevokeAsync(GrantId, RevocationReason)` only                                                                                                                                                                                     |
| `IToolAuthorizer`                                                                                                                                                                                                                                                                                                                           | reduced stand-in        | `Abstractions/Tools/IToolAuthorizer.cs:11-16`; WS4 removes it                                                                                                                                                                     |
| validator                                                                                                                                                                                                                                                                                                                                   | WIRED (WS3-C12)         | Singular selector, catalog, broker, audit dispatcher, approval store, decision store                                                                                                                                              |
...
| `SecurityApprovalRequired`                                                                                                                   | `:140-143`                                   | SPEC, implemented in WS3-C1                                                                                                                                                                                                                   |
| `SecurityGrant` spec shape                                                                                                                   | `:145-160`                                   | SPEC; convergence with current shape is a separate review                                                                                                                                                                                     |
| `ApprovalValidityWindow`, `SecurityRevocationConstraint`, `ApprovalAuthenticationEvidence`, approval request/response, `SecurityAuditRecord` | `:170-230`                                   | SPEC, implemented in WS3-C1; `ApprovalAuthenticationMethod` interim block added after `ISecurityAuthority` (WS3-C1); `SecurityPresentation` still NO-SPEC                                                                                     |
| `ISecurityPolicy.EvaluateAsync(request, SecurityPolicyContext, ct)`                                                                          | `:247-253`                                   | SPEC; `SecurityPolicyContext` interim type added in WS3-C1, un-wired until C3                                                                                                                                                                 |
| `ISecurityPolicyCatalog`, `ISecurityPolicySelector`                                                                                          | `:269-281`                                   | SPEC, implemented in WS3-C1; `SecurityPolicySnapshotResult` interim type (3 cases) added in WS3-C1                                                                                                                                            |
| `ISecurityAuthority.AuthorizeAsync(request, HookDispatchContext?, ct)`                                                                       | `:290-296`                                   | SPEC                                                                                                                                                                                                                                          |
| handler dispatcher, broker `ResolveAsync`, store create/resolve, `RevokeAsync(GrantId, RevocationReason)`                                    | `:404-451`                                   | SPEC; `IApprovalHandlerDispatcher` implemented in WS3-C1; `RevocationReason`, `GrantRevocationResult` (4 cases), and `ApprovalResolutionResult` (5 cases) added as interim types after `ISecurityGrantStore` (WS3-C1), un-wired until C7a/C8b |
| `SecurityAuthority` and `ApprovalBroker` ctors                                                                                               | `:497-515`                                   | SPEC; `ISecurityGrantIssuer`, `ISecurityDecisionStore` added as interim types in WS3-C1, un-wired until C6a                                                                                                                                   |
...
- Landed: every deliverable added as a standalone additive type with zero
  production consumers, matching this chunk's own scope. `ApprovalChannelId`,
  `ApprovalAuthenticationEvidenceId`, `SecurityRevocationTrigger`,
  `SecurityRevocationConstraint`, `ApprovalValidityWindow`,
  `ApprovalAuthenticationEvidence`, and `SecurityApprovalRequired` matched the
  existing `permissions-and-human-control.md` C# blocks exactly.
  `ApprovalAuthenticationMethod`, `SecurityPolicyContext`,
  `SecurityAllowConstraints`, `RevocationReason`, the four-case
  `GrantRevocationResult` family, `ISecurityGrantIssuer`,
  `ISecurityDecisionStore`, the three-case `SecurityPolicySnapshotResult`
  family, and the five-case `ApprovalResolutionResult` family were NO-SPEC;
  their C# blocks were written into `permissions-and-human-control.md` in this
  chunk, after the `ISecurityAuthority` interface and after
  `ISecurityGrantStore` respectively. `ISecurityPolicyCatalog`,
  `ISecurityPolicySelector`, and `IApprovalHandlerDispatcher` reused the
  signatures already shown in that document. Records containing an
  `ImmutableArray<T>` member (`SecurityRevocationConstraint`,
  `SecurityAllowConstraints`) override `Equals`/`GetHashCode` with
  `SequenceEqual`, matching `SecurityGrant` and `SecurityRequest`.
  `ISecurityGrantIssuer.IssueAsync` takes the request, policy/revocation
  versions, and an optional `ApprovalScopeBinding` directly rather than
  introducing an additional request record, since that is exactly the input
  `SecurityAuthority.AuthorizeCoreAsync` already assembles before minting a
  grant inline. `GrantRevocationResult` and `ApprovalResolutionResult` are not
  yet wired into `ISecurityGrantStore.RevokeAsync` or
  `IApprovalBroker.ResolveAsync` (C7a and C8b respectively). New guard tests
  live under `tests/AgentKit.Abstractions.Tests/Security` and `.../Identity`;
  `SecurityDecisionTests` gained the `SecurityApprovalRequired` cases alongside
  its existing `SecurityAllowed`/`SecurityDenied` coverage rather than a new
  file, matching this repository's one-file-per-family test convention.
