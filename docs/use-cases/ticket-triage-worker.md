...
and any in-flight tool, the run settles as `RunCancelled`, and the ticket is
dead-lettered instead of updated from a half-formed answer. The per-job
...
- **A cap is a typed stop.** When a reservation is refused with full evidence
  the run settles as `RunLimitReached` naming the dimension; the worker sees a
  failed outcome and dead-letters the ticket instead of an exception.
