...
| -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| [Customer support assistant in a web API](web-support-assistant.md)        | Durable per-customer conversations, identity from your authentication, resuming a session, streaming over SSE.      |
...
| [Background ticket-triage worker](ticket-triage-worker.md)                 | A hosted worker that builds one engine per job with limits, timeouts, cancellation, telemetry, and required audit.  |
...
| [Delegating to specialist agents](delegating-to-specialists.md)            | The `task` tool, the delegation broker, and a host channel that runs a child agent and returns a bounded result.    |
