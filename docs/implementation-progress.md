...
| AgentKit.Simple builder                                | New leaf `AgentKit.Simple`: `Use*`/`With*` extensions on `AgentEngineBuilder` and `AskAsync` on `AgentEngine` compose the real packages with named local-development defaults; `AgentEngine.Services` added (mirrors `IHost.Services`); `UseSqliteSessions` and `UseWorkspace` with restart-resume and sandbox-refusal proofs; 46 focused cases plus 8 QuickStart tests; human-oriented storage, file-system, and permissions guides; `AgentKit.Session.Sqlite` and its tests added to the solution | Verified checkpoint; sugar over public registrations, no second runtime; `UseOpenAI` is the only provider sugar, others go through `Services` + `UseModel` |
| Known-model catalog                                    | `KnownModelCatalog` in `AgentKit.Providers` with an embedded resource of 128 models across 10 adapter-backed providers, generated from `openclaw/catalog`; `AddOpenAIKnownLlmModel`                                                                                                                                                                                                                                                                                                                 | Verified checkpoint; reference data with stated provenance, not runtime discovery; other provider packages can add the same one-call registration          |
...
`IDurableOperationCodec<TState>` generically: each instance is bound to one
exact operation name/version pair, stamps that version as the payload's schema
version, and rejects a mismatched or oversized payload as
`DurableDecodeIncompatible` before attempting to parse it. It lives in
`AgentKit.Abstractions` alongside `StaticApiKeyCredentialSource`, following the
same precedent of a generic, dependency-free default living with its contract.

`InMemoryDurableOperationJournal` implements `IDurableOperationJournal` in
`AgentKit.Durability.InMemory`. It records acceptance, checkpoints, and terminal
results per address, enforces exactly the fencing rule the interface requires,
and computes `DurableOperationState` (and the `StartDefinitelyAbsent` and
`SideEffectCertainty` evidence derived from it) from which method committed most
recently rather than trusting a separate caller claim. It does not perform grant
consumption or audit dispatch: none of the interface's four methods receives a
live `SecurityGrant`/`SecurityEnforcementIntent`, and `LoadEvidenceAsync`
receives only a bare address with no authorization evidence at all. This is a
specification gap in `IDurableOperationJournal` itself, not an omission in this
adapter, and is documented on the class and in this ledger rather than papered
over with a protection the interface cannot support. The same interface shape
also cannot record `Waiting` state or an `ExternalOperationReference`, since no
method accepts either.

Verification: the Release solution builds with zero warnings or errors and all
7,841 tests pass with no failures or skips, up from 7,792. The checkpoint adds
68 cases: 13 for the codec in `AgentKit.Abstractions.Tests`, and 55 for the
journal, its diagnostics, and registration in
`AgentKit.Durability.InMemory.Tests`. Seven mutations across both — disabling
the codec's schema-version check, its encode and decode byte bounds, the
journal's fencing rejection, its terminal-blocks-checkpoint guard, its
idempotent-vs-conflicting terminal-replay check, and its computed
`StartDefinitelyAbsent` value — were each observed failing at least one
owning-class test before the implementation was restored. The three reviewed API
snapshot changes are additive: the new codec type, the new journal type and its
registration method, and six new shared observability names.
...
- A first-party `IRecoveryPolicy` is blocked by a second unresolved
  ...
  choice.
- Correction to the previous entry: `IDurableOperationCodec<TState>` and
  `IDurableOperationJournal` are no longer unimplemented.
  `JsonDurableOperationCodec<TState>` is a first-party generic JSON default (an
  application still authors its own codec when it needs guaranteed
  forward-compatible field retention beyond what `TState`'s own shape captures).
  `InMemoryDurableOperationJournal` records acceptance, checkpoints, and
  terminal results with fencing exactly as required; it does not perform grant
  consumption or audit dispatch, because `IDurableOperationJournal`'s four
  methods do not receive a live grant/intent the way `ISessionStore`'s protected
  methods do, and `LoadEvidenceAsync` receives no authorization evidence at all.
  This is a specification gap, not effort remaining in this adapter; closing it
  is an architecture decision for
...
not make that component a mandatory dependency of every engine. The ordered work
to close every row is in the [implementation plan](implementation-plan.md); rows
are updated as each workstream lands.
...
| Composition and configuration | Verified: engine-level admission through `Agent.SendAsync` (session create/open with ownership checks, durable session-lane discovery/provisioning/input-admission/run-acceptance through `LoadLaneStateAsync`/`ProvisionLaneAsync`/`AdmitInputAsync`/`AcceptRunAsync` layered behind a process-local per-session concurrency gate applying `SessionBusyBehavior`, keyed run compilation, engine-driven best-effort lane release on settlement, observer wiring) and multi-agent hosting through `AgentKit.Simple.AddAgent`; remaining: queue-backed steering/follow-up admission during an in-flight run, run attachment/cancellation by `RunId`, unified outcome family, catalog publication/reload, readiness                                                                                                                                                                     |
| Agent runtime                 | Verified: history reconstruction from the newest active compaction checkpoint (summary projected as a `RuntimeMessage` plus exact retained suffix, real-tip cursor, covered-range recovery exclusion, content-free event 1091). Outstanding: explicit state transitions, waiter cancellation, recovery identity, settlement outcomes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Budgets                       | Verified: loop consumption (run scope per budgeted run; turn, model-request, and tool-call reservations; post-response token and cost accounting; typed `AgentRunBudgetExhausted`; fail-closed without an authority) and `AgentKit.Simple.WithBudget`; SQLite ledger checkpoint verified; remaining: pre-effect unknown-cost estimation, named profiles, parent host/tenant scopes, policy/event contracts, full conformance                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Messages and history          | Immutable/loss-aware values, non-elevation, correlation and shared round-trip conformance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Input and output              | Verified: `SessionBackedInputQueue` (`AgentKit.IO`), the first-party `IInputQueue` persisting admission and mid-run promotion through the session store's lane protocol, unit-tested against a scripted `ISessionCoordinator`. Remaining: not reachable end to end (no engine wiring resolves `SessionExecutionCapability`/`IInputCoordinator` into a real run scope, `DefaultAgentLoop` does not yet call `PromoteAsync` at any turn boundary, and no engine entry point admits steering/follow-up input into an in-flight run), fan-out, final publication and channel contracts, `IRunEventSink`/`DefaultOutputPublisher`/`IOutputBackpressurePolicy`                                                                                                                                                                                                                             |
| Structured output             | Verified: loop integration (terminal-response validation, per-run repair accounting through a `RuntimeMessage`, typed rejection halts, fail-closed without a processor), conversation surfacing and `AgentKit.Simple` `WithOutput<T>`/`AskAsync<T>`; remaining: provider-native schema negotiation, synthetic-tool/media/union modes, validation-retry budget reservation, hook points                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Context                       | Instruction precedence, contributor trust/order, bounded assembly and request manifests                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Context compaction            | Verified: SourceThrough enforcement, pinned/version-observed source reads, CompactionId reconciliation, truthful cancellation commit state, cut/manifest coherence validation, user-turn cut preference, deadline enforcement, model-backed strategy (`ModelCompactionStrategy`: catalog/selector/resolver selection in the captured scope, configurable `SummaryPrompt` with embedded default, bounded transcript and summary, typed failure mapping, model provenance, content-free signals); loop context-pressure trigger with checkpoint reload and `AgentKit.Simple.WithCompaction`. Outstanding: provider-overflow and instruction-epoch triggers, `ICompactionSummaryGenerator` seam and budget threading for the model-backed strategy, full policy snapshot, session-wide sequence allocation across branches                                                              |
...
| Sessions                      | `.InMemory`, `.Sqlite`, and `.Json` stores verified with a shared conformance suite, including `ISessionStore.LoadLaneStateAsync`/`ISessionCoordinator.LoadLaneStateAsync` for discovering a provisioned lane's current revision, branch cursor, and optional accepted run without a second non-durable copy of that state, and `ISessionStore.LoadPendingInputsAsync`/`PromoteInputAsync` for discovering and atomically committing durably admitted input into an already-accepted run's current turn. `AgentEngine.SendAgentAsync` drives the full initial-admission lane protocol end to end and `DefaultAgentLoop` is lane-aware (uses the admitted lane and `TurnId`, releases on settlement). Outstanding: branch fencing beyond optimistic version checks, retention/export/import, engine/loop wiring that actually calls the mid-run promotion path (see Input and output) |
...
| Goals and delegation          | Verified: delegation broker grant enforcement and the engine-backed in-process channel (`EngineDelegationChannel`, `AgentKit.Simple.WithDelegation`); remaining: durable goal and attempt records, joins, detached worker hosting                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Hooks and extensions          | Verified: typed dispatch kernel with ordering, mutation validation, isolation rollback, and reentrancy bounds; first-party points `RunStarted`, `BeforeModelRequest`, and `BeforeToolInvocation` dispatched by the loop with per-point registration; remaining: profile/catalog selection, `HookDispatchMetadata` shape, further points, timeout quiescence                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
