...
| Sessions                      | `.InMemory`, `.Sqlite`, and `.Json` stores verified with a shared conformance suite, including `ISessionStore.LoadLaneStateAsync`/`ISessionCoordinator.LoadLaneStateAsync` for discovering a provisioned lane's current revision, branch cursor, and optional accepted run without a second non-durable copy of that state; `AgentEngine.SendAgentAsync` now drives the full lane protocol end to end and `DefaultAgentLoop` is lane-aware (uses the admitted lane and `TurnId`, releases on settlement). Outstanding: branch fencing beyond optimistic version checks, retention/export/import, a durable `IInputQueue`/`IInputCoordinator` path for mid-run steering and follow-up admission                                                                                                                          |
