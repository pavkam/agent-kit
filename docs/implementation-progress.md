...
| AgentKit.Simple builder                                | New leaf `AgentKit.Simple`: `SimpleAgentBuilder`/`SimpleAgent` compose the real packages with named local-development defaults; 34 focused cases plus 8 QuickStart tests; `AgentKit.Session.Sqlite` and its tests added to the solution | Verified checkpoint; sugar over public registrations, no second runtime; `UseOpenAI` is the only provider sugar, others go through `Services` + `UseModel` |
