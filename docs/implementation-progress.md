...
| Context compaction            | Verified: SourceThrough enforcement, pinned/version-observed source reads, CompactionId reconciliation, truthful cancellation commit state, cut/manifest coherence validation, user-turn cut preference, deadline enforcement, model-backed strategy (`ModelCompactionStrategy`: catalog/selector/resolver selection in the captured scope, configurable `SummaryPrompt` with embedded default, bounded transcript and summary, typed failure mapping, model provenance, content-free signals). Outstanding: `ICompactionSummaryGenerator` seam and budget threading for the model-backed strategy, full policy snapshot, session-wide sequence allocation across branches |
