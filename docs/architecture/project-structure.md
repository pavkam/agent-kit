...
| Budgets                  | `IBudgetAuthority`, `IBudgetScope`, reservations, limits, usage                              | AgentKit.Budgets / `AddAgentBudgets`; explicit `AgentKit.Budgets.InMemory`, `AgentKit.Budgets.Sqlite`, or `AgentKit.Budgets.Json` ledger leaf                      | Engine/tenant authority with run- and operation-owned child scopes and reservations                              |
...
| Security                 | authority, policies, approvals, grants, audit contracts                                      | AgentKit.Permissions / `AddAgentPermissions`; explicit `AgentKit.Permissions.InMemory`, `AgentKit.Permissions.Sqlite`, or `AgentKit.Permissions.Json` store leaves | One effective authority/policy/approval selection per definition; additive policies/handlers/sinks               |
...
- `AgentKit.<Owner>.Sqlite` supplies durable local storage for the subset of the
  contract SQLite can implement truthfully;
- `AgentKit.<Owner>.Json` supplies durable single-writer local storage in
  inspectable JSON documents and newline-delimited record logs, sharing its file
  mechanics, fingerprinted encoding contract, and portable evidence shapes
  through the `AgentKit.Storage.Json` machinery package; and
...
Every storage family supplies one reusable conformance suite. Its in-memory,
SQLite, and JSON leaves run the same common behavioral cases, while
adapter-specific tests prove only advertised durability, transaction, isolation,
fencing, pagination, and migration guarantees. SQLite is a durable local
database, not evidence of a distributed lease or atomicity with another
database, filesystem, provider, or external effect. A JSON leaf is a durable
single-writer local log: it flushes every acknowledged record, recovers a torn
trailing append, and rejects a second writer, which is weaker than SQLite's
multi-process coordination and must be advertised as such.
...
| AgentKit.Session.Json                | AgentKit.Session.Json.Tests                |
...
| AgentKit.Permissions.Json            | AgentKit.Permissions.Json.Tests            |
| AgentKit.Budgets.InMemory            | AgentKit.Budgets.InMemory.Tests            |
| AgentKit.Budgets.Sqlite              | AgentKit.Budgets.Sqlite.Tests              |
| AgentKit.Budgets.Json                | AgentKit.Budgets.Json.Tests                |
| AgentKit.Storage.Json                | AgentKit.Storage.Json.Tests                |
