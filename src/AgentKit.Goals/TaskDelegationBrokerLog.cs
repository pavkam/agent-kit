...
internal static partial class TaskDelegationBrokerLog
{
    ...
    [LoggerMessage(EventId = 2000, Message = "Task delegation {DelegationId} dispatch completed with {Outcome}. Tenant {TenantId} agent {AgentId} session {SessionId} run {RunId} turn {TurnId} tool {ToolCallId} operation {OperationId} security request {SecurityRequestId}.")]
    ...
    [LoggerMessage(EventId = 2001, Level = LogLevel.Information, Message = "Task delegation {DelegationId} dispatch wait was cancelled. Tenant {TenantId} agent {AgentId} session {SessionId} run {RunId} turn {TurnId} tool {ToolCallId} operation {OperationId} security request {SecurityRequestId}.")]
    ...
    [LoggerMessage(EventId = 2002, Level = LogLevel.Error, Message = "Task delegation {DelegationId} dispatch failed with error type {ErrorType}. Tenant {TenantId} agent {AgentId} session {SessionId} run {RunId} turn {TurnId} tool {ToolCallId} operation {OperationId} security request {SecurityRequestId}.")]
