...
/// <summary>Mutable composition-time input for the operational and evidence bounds of one SQLite approval-store leaf.</summary>
/// <remarks>
/// Instances exist only while a configure delegate passed to
/// <see cref="ServiceExtensions.AddSqliteApprovalStore(IServiceCollection, SqliteApprovalStoreTarget, Action{SqliteApprovalStoreOptions}?)"/>
/// runs. The registration materializes the mutated values into an immutable, eagerly validated
/// <see cref="SqliteApprovalStoreSettings"/> and never registers this type in dependency injection.
/// </remarks>
public sealed class SqliteApprovalStoreOptions
{
    ...
    /// <summary>Gets or sets the maximum encoded request or response payload size.</summary>
