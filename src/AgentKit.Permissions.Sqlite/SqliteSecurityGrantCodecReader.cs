...
internal ref struct SqliteSecurityGrantCodecReader
{
    ...
    internal SqliteSecurityGrantCodecReader(ReadOnlySpan<byte> payload, SqliteSecurityGrantStoreSettings settings)
    {
    ...
    internal Guid ReadGuid() => SqliteSecurityGrantCodec.DecodeGuid(ReadBytes(16));
    ...
    internal ExecutionIdentity ReadIdentity()
    {
        ...
        var delegationCount = ReadBoundedCount(_settings.MaximumDelegationLinks, 64);
    ...
    {
        var count = ReadBoundedCount(_settings.MaximumResources, 8);
        if (count == 0)
        {
    ...
    {
        var count = ReadBoundedCount(_settings.MaximumClaims, 16);
