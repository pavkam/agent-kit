...
/// <summary>Publishes the agent definitions described by a <see cref="SimpleAgentPlan"/>, advertising every allowed registered tool.</summary>
...
internal sealed class SimpleAgentDefinitionSource: IAgentDefinitionSource
{
    ...
    /// <param name="toolOptions">The tool runtime's authorization options; tools the allow-list excludes are not advertised.</param>
    /// <exception cref="ArgumentNullException"><paramref name="plan"/>, <paramref name="tools"/>, or <paramref name="toolOptions"/> is null.</exception>
    ...
    public SimpleAgentDefinitionSource(SimpleAgentPlan plan, IEnumerable<ITool> tools, IOptions<AgentToolsOptions> toolOptions)
    {
        ArgumentNullException.ThrowIfNull(plan);
        ArgumentNullException.ThrowIfNull(tools);
        ArgumentNullException.ThrowIfNull(toolOptions);
        plan.Validate();
        var definitions = SimpleAgentPlan.AdvertisedTools(tools, toolOptions.Value).ToLlmToolDefinitions();
