...
/// guarantees that the caller must not begin the protected effect. Valid
/// instances are restricted to those built-in variants: the parameterless
/// constructor cannot be used outside this assembly, and the guarded
/// record-copy constructor prevents an external derived record from
/// bootstrapping itself from a built-in variant.
/// </remarks>
public abstract record BudgetStartResult
{
    /// <summary>
    /// Initializes a built-in result variant without copying an existing
    /// result.
    /// </summary>
    /// <remarks>
    ...
    /// result.
    /// </remarks>
    protected BudgetStartResult(BudgetStartResult original)
    {
