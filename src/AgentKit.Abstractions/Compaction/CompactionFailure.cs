// Copyright (c) AgentKit contributors. All rights reserved.
// Licensed under the MIT License. See LICENSE in the project root for license information.

namespace AgentKit;

/// <summary>
/// An unexpected failure encountered during a compaction attempt, distinct
/// from a deliberate <see cref="CompactionRejection"/>.
/// </summary>
/// <remarks>
/// This type is an immutable value object with structural equality over its
/// fields, safe to share across threads without synchronization.
/// <see cref="SafeMessage"/> never contains source session content;
/// unredacted diagnostic detail belongs in an observability sink correlated
/// by the originating operation, not on this record.
/// </remarks>
public sealed record CompactionFailure
{
    /// <summary>Initializes a new instance of the <see cref="CompactionFailure"/> record.</summary>
    /// <param name="kind">The category of this failure.</param>
    /// <param name="safeMessage">A human-readable, non-sensitive explanation.</param>
    /// <param name="retryable">Whether retrying this attempt unchanged might succeed.</param>
    /// <param name="details">Additional caller-specific or forward-compatible detail.</param>
    /// <exception cref="ArgumentException">
    /// <paramref name="safeMessage"/> is null, empty, or consists only of
    /// whitespace.
    /// </exception>
    /// <exception cref="ArgumentNullException"><paramref name="details"/> is null.</exception>
    public CompactionFailure(CompactionFailureKind kind, string safeMessage, bool retryable, ExtensionData details)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(safeMessage);
        ArgumentNullException.ThrowIfNull(details);

        Kind = kind;
        SafeMessage = safeMessage;
        Retryable = retryable;
        Details = details;
    }

    /// <summary>Gets the category of this failure.</summary>
    public CompactionFailureKind Kind { get; init; }

    /// <summary>Gets a human-readable, non-sensitive explanation.</summary>
    public string SafeMessage { get; init; }

    /// <summary>Gets a value indicating whether retrying this attempt unchanged might succeed.</summary>
    public bool Retryable { get; init; }

    /// <summary>Gets additional caller-specific or forward-compatible detail.</summary>
    public ExtensionData Details { get; init; }
}
