...
/// <summary>Selects one named goal, attempt, delegation, and worker-policy profile.</summary>
/// <remarks>This immutable value uses ordinal text equality and identifies configuration only; it does not create work or authorize delegation. Values are preserved exactly without trimming or normalization and compare with ordinal, case-sensitive semantics. A default instance has no usable selection text; consumers must reject it at their boundary.</remarks>
public readonly record struct GoalProfileKey
{
    /// <summary>Initializes a validated goal-profile selection key.</summary>
    ...
    public GoalProfileKey(string value)
    {
