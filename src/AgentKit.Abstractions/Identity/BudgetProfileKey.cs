...
/// <summary>Selects one named hierarchy of run-budget limits and accounting policy.</summary>
/// <remarks>This immutable value uses ordinal text equality and carries selection identity only; it does not reserve or grant budget. Values are preserved exactly without trimming or normalization and compare with ordinal, case-sensitive semantics. A default instance has no usable selection text; consumers must reject it at their boundary.</remarks>
public readonly record struct BudgetProfileKey
{
    /// <summary>Initializes a validated budget-profile selection key.</summary>
    ...
    public BudgetProfileKey(string value)
    {
