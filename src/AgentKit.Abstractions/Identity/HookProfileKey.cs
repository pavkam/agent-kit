...
/// <summary>Selects one named immutable hook registration and failure-policy profile.</summary>
/// <remarks>This immutable value uses ordinal text equality and names configuration only; it does not activate hooks or convey authority. Values are preserved exactly without trimming or normalization and compare with ordinal, case-sensitive semantics. A default instance has no usable selection text; consumers must reject it at their boundary.</remarks>
public readonly record struct HookProfileKey
{
    /// <summary>Initializes a validated hook-profile selection key.</summary>
    ...
    public HookProfileKey(string value)
    {
