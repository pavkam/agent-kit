// Copyright (c) AgentKit contributors. All rights reserved.
// Licensed under the MIT License. See LICENSE in the project root for license information.

namespace AgentKit;

/// <summary>
/// Identifies one immutable <see cref="AgentMessage"/> in durable session
/// history. Every message a session ever appends — user input, assistant
/// output, tool results, system records, synthetic runtime notices — gets
/// exactly one <see cref="MessageId"/> for its entire lifetime.
/// </summary>
/// <remarks>
/// <para>
/// This type is an immutable value object with structural equality over
/// <see cref="Value"/>. It carries no mutable state itself and is safe to
/// share, compare, and use as a lookup key across threads without
/// synchronization.
/// </para>
/// <para>
/// Because durable history is append-only, a <see cref="MessageId"/> is
/// permanent: once a message is committed, its identity never changes, is
/// never reused for a different message, and never moves to a different
/// position implicitly. Repairs, compaction, and branching reference
/// existing message identities rather than mutating or reassigning them, so
/// causal relationships such as "this tool result answers that call" or
/// "this summary replaces these messages" stay resolvable through IDs
/// instead of array position or text matching.
/// </para>
/// <para>
/// As with every AgentKit identity struct, the CLR default value (an
/// all-zero GUID) can still exist as an uninitialized field's value; this
/// constructor rejects that value for every explicitly constructed instance
/// so a valid <see cref="MessageId"/> always addresses a real message.
/// </para>
/// </remarks>
public readonly record struct MessageId
{
    /// <summary>
    /// Initializes a new instance of the <see cref="MessageId"/> struct,
    /// validating that it addresses a real message rather than an empty
    /// placeholder.
    /// </summary>
    /// <param name="value">The non-empty underlying globally unique identifier.</param>
    /// <exception cref="ArgumentOutOfRangeException">
    /// <paramref name="value"/> is <see cref="Guid.Empty"/>, which can never
    /// address a real message.
    /// </exception>
    public MessageId(Guid value)
    {
        ArgumentOutOfRangeException.ThrowIfEqual(value, Guid.Empty, nameof(value));
        Value = value;
    }

    /// <summary>
    /// Gets the underlying globally unique identifier. This is the only
    /// state the type carries, and it never changes after construction.
    /// </summary>
    public Guid Value { get; }

    /// <summary>
    /// Returns the canonical, lowercase, hyphenated text form of this
    /// identity (GUID "D" format), suitable for logging, storage keys, and
    /// round-tripping through configuration or external protocols.
    /// </summary>
    public override string ToString() => Value.ToString("D");
}
