...
/// <summary>
/// The agent exists but its definition cannot be used with the current
/// composition.
/// </summary>
/// <remarks>
/// This is distinct from <see cref="AgentDefinitionNotFound"/>: the identity
/// is real, but something it selects is missing, ambiguous, or incompatible.
/// It is a configuration error to fix rather than a client mistake, and the
/// diagnostics say which part is wrong.
/// </remarks>
public sealed record InvalidAgentDefinition: AgentDefinitionResolution
{
    private readonly ImmutableArray<CompositionDiagnostic> _diagnostics;

    /// <summary>
    /// Initializes a new instance of the <see cref="InvalidAgentDefinition"/>
    /// record.
    /// </summary>
    /// <param name="agentId">The agent whose definition is unusable.</param>
    /// <param name="diagnostics">
    /// Why the definition is unusable. At least one diagnostic is required,
    /// because an unexplained invalid definition is not actionable.
    /// </param>
    /// <exception cref="ArgumentException">
    /// <paramref name="diagnostics"/> is uninitialized, empty, or contains
    /// <see langword="null"/>.
    /// </exception>
    public InvalidAgentDefinition(
        AgentId agentId,
