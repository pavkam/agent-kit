// Copyright (c) AgentKit contributors. All rights reserved.
// Licensed under the MIT License. See LICENSE in the project root for license information.

namespace AgentKit.Session.Json;

/// <summary>Captures immutable positive bounds, continuation limits, compaction policy, and encoding contract for one JSON session store.</summary>
public sealed record JsonSessionStoreSettings
{
    /// <summary>Initializes explicit bounds, continuation limits, compaction policy, and the frozen JSON encoding contract.</summary>
    /// <param name="maximumRecordBytes">The positive maximum encoded size of one persisted transition record.</param>
    /// <param name="maximumDocumentBytes">The positive maximum encoded size of the store manifest document.</param>
    /// <param name="compactionRecordThreshold">The positive replayed-record count above which initialization rewrites the log to its live state.</param>
    /// <param name="maximumIssuedReadSnapshots">The positive number of distinct adapter-issued read snapshots retained in process for exact paged continuation.</param>
    /// <param name="encoding">The frozen JSON encoding contract bound to the store root.</param>
    /// <exception cref="ArgumentNullException"><paramref name="encoding"/> is null.</exception>
    /// <exception cref="ArgumentOutOfRangeException">A byte bound, the compaction threshold, or the snapshot bound is not positive.</exception>
    public JsonSessionStoreSettings(
        int maximumRecordBytes,
        int maximumDocumentBytes,
        int compactionRecordThreshold,
        int maximumIssuedReadSnapshots,
        JsonEncodingSettings encoding)
    {
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(maximumRecordBytes);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(maximumDocumentBytes);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(compactionRecordThreshold);
        ArgumentOutOfRangeException.ThrowIfNegativeOrZero(maximumIssuedReadSnapshots);
        ArgumentNullException.ThrowIfNull(encoding);

        MaximumRecordBytes = maximumRecordBytes;
        MaximumDocumentBytes = maximumDocumentBytes;
        CompactionRecordThreshold = compactionRecordThreshold;
        MaximumIssuedReadSnapshots = maximumIssuedReadSnapshots;
        Encoding = encoding;
    }

    /// <summary>Gets the maximum encoded transition-record size.</summary>
    /// <value>
    /// A positive byte count enforced before an append and while replaying the log. One record carries a complete accepted
    /// request, so a batch append of large entries is bounded here as well as by the entry codec's own payload ceiling.
    /// </value>
    public int MaximumRecordBytes { get; }

    /// <summary>Gets the maximum encoded manifest size.</summary>
    /// <value>A positive byte count enforced before the manifest document is decoded.</value>
    public int MaximumDocumentBytes { get; }

    /// <summary>Gets the replayed-record count that triggers compaction.</summary>
    /// <value>
    /// A positive count compared against the number of records replayed at initialization. Exceeding it rewrites the log
    /// from live state so replay cost stays bounded as sessions accumulate transitions over the root's lifetime.
    /// </value>
    public int CompactionRecordThreshold { get; }

    /// <summary>Gets how many adapter-issued paged-read snapshots one store instance retains for exact continuation.</summary>
    /// <value>
    /// A positive count. Issued snapshots are process-local provenance rather than durable state, so once the bound is
    /// exceeded the oldest is evicted and continuing from it fails as unavailable instead of being trusted as
    /// caller-authored evidence. Restarting the process also discards them.
    /// </value>
    public int MaximumIssuedReadSnapshots { get; }

    /// <summary>Gets the frozen JSON encoding contract.</summary>
    /// <value>
    /// The immutable host-configured contract. The store layers its required polymorphism resolver, identity converters,
    /// and session-entry envelope converter onto it and binds the resulting fingerprint to the root at initialization.
    /// </value>
    public JsonEncodingSettings Encoding { get; }

    /// <summary>Creates conservative explicit defaults for local applications.</summary>
    /// <returns>Settings with one-mebibyte payload bounds, a 4096-record compaction threshold, 4096 retained read snapshots, and the canonical encoding contract.</returns>
    public static JsonSessionStoreSettings CreateDefault() => new(
        1_048_576,
        1_048_576,
        4_096,
        4_096,
        JsonEncodingSettings.CreateDefault());
}
