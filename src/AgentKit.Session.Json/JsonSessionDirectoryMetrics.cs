...
/// <summary>Owns the bounded terminal-outcome counter emitted by the durable JSON session-directory adapter.</summary>
/// <remarks>
/// The counter reuses the shared session-directory metric name so dashboards aggregate across every directory adapter.
/// Only the bounded operation name and the fixed outcome vocabulary are used as dimensions; no tenant, agent, or session
/// identity enters a metric.
/// </remarks>
internal static class JsonSessionDirectoryMetrics
{
    private static readonly Counter<long> _operations = AgentKitDiagnostics.Metrics.CreateCounter<long>(
        AgentKitMetricNames.SessionDirectoryOperationCount,
        unit: "{operation}",
        description: "Number of terminal durable JSON session-directory outcomes.");

    /// <summary>Records one directory outcome with bounded operation and result dimensions.</summary>
