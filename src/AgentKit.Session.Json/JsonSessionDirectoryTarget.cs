...
/// <summary>Captures one fixed local JSON directory root supplied through the session-routing bootstrap boundary.</summary>
/// <remarks>
/// The value authorizes no ordinary file operation. Construction validates configuration without probing or creating the
/// root; initialization performs the explicitly selected effects. The root is owned exclusively by the session directory:
/// it holds its own manifest and advisory lock and must not be shared with the session store or another storage family.
/// </remarks>
public sealed record JsonSessionDirectoryTarget
{
    /// <summary>Initializes immutable bootstrap configuration for one exact directory root.</summary>
    ...
    /// rejected for the same reason as the store target: permitting creation already accepts initialization writes, and
    /// refusing recovery would leave a crashed directory permanently unopenable.
    /// </remarks>
    public JsonSessionDirectoryTarget(
        string directoryPath,
        JsonSessionDirectoryInstanceId expectedStoreInstanceId,
        ...
        JsonStoreRecoveryMode recoveryMode)
    {
        ...
        if (!Path.IsPathFullyQualified(directoryPath))
        {
            throw new ArgumentException(
                "The JSON session-directory root must be a fully qualified path.", nameof(directoryPath));
        }
    ...
    /// <summary>Gets the normalized fully qualified directory root.</summary>
    ...
    /// <summary>Gets the expected persistent directory identity.</summary>
    /// <value>The exact nondefault identity verified against the manifest during initialization.</value>
    public JsonSessionDirectoryInstanceId ExpectedStoreInstanceId { get; }
    ...
    /// <value>The explicit validation or recovery mode applied to the routing record log during initialization.</value>
