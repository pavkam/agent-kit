...
/// <summary>Defines content-free source-generated events for the durable JSON session-store adapter.</summary>
/// <remarks>
/// Event identities are owned by this package and stable across releases. Only bounded operation names, typed domain
/// identities, and a fixed outcome vocabulary are emitted; session entries, payloads, and the configured root path are
/// never logged.
/// </remarks>
internal static partial class JsonSessionStoreLog
{
    ...
    [LoggerMessage(19300, LogLevel.Debug, "JSON session store operation {Operation} for agent {AgentId} and session {SessionId} completed with outcome {Outcome}.")]
    internal static partial void Completed(
        ILogger logger, string operation, AgentId agentId, SessionId? sessionId, string outcome);

    /// <summary>Records an unexpected store failure type without exception, entry, or path content.</summary>
    /// <param name="logger">The configured Microsoft logger.</param>
    /// <param name="operation">The bounded store operation name.</param>
    /// <param name="agentId">The owning agent identity.</param>
    /// <param name="sessionId">The session identity when already established.</param>
    /// <param name="errorType">The stable exception type name.</param>
    [LoggerMessage(19301, LogLevel.Error, "JSON session store operation {Operation} for agent {AgentId} and session {SessionId} failed with error type {ErrorType}.")]
