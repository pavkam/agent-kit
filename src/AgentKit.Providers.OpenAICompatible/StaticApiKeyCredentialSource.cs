...
/// static, long-lived API key, shared by every OpenAI-compatible provider
/// package that authenticates with a plain API key.
/// </summary>
/// <remarks>
/// This is the simplest and most common authentication mode for
/// OpenAI-compatible endpoints: the application configures a single API key
/// (typically from a secret store), and every request uses it unchanged for
/// the lifetime of this instance. An application that needs to rotate keys
/// without restarting the process, or that authenticates with a refreshable
/// OAuth token instead, registers <see cref="DelegatingOAuthCredentialSource"/>
/// in place of this type rather than trying to make this type do both.
/// </remarks>
