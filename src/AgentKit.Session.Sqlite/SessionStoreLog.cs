...
/// <summary>Defines allocation-efficient SQLite session-store events without entry or identity content.</summary>
internal static partial class SessionStoreLog
{
    ...
    [LoggerMessage(25000, LogLevel.Debug, "SQLite session operation {Operation} for agent {AgentId} and session {SessionId} completed with outcome {Outcome}.")]
    ...
    [LoggerMessage(25001, LogLevel.Error, "SQLite session operation {Operation} for agent {AgentId} and session {SessionId} failed with error type {ErrorType}.")]
