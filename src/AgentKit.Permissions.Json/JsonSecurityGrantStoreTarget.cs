...
/// <summary>Captures one fixed local JSON store root supplied through the security control-plane bootstrap boundary.</summary>
/// <remarks>
...
/// </remarks>
public sealed record JsonSecurityGrantStoreTarget
{
    ...
    /// </remarks>
    public JsonSecurityGrantStoreTarget(
        string directoryPath,
        JsonSecurityGrantStoreInstanceId expectedStoreInstanceId,
        ...
        JsonStoreRecoveryMode recoveryMode)
    {
        ...
        if (!Path.IsPathFullyQualified(directoryPath))
        {
            throw new ArgumentException(
                "The JSON grant-store root must be a fully qualified path.", nameof(directoryPath));
        }
    ...
    /// <value>The exact nondefault identity verified against the manifest before every grant-store access.</value>
    public JsonSecurityGrantStoreInstanceId ExpectedStoreInstanceId { get; }
    ...
    /// <value>The explicit validation or recovery mode applied to each record log during initialization.</value>
