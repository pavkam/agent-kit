...
/// <summary>Mutable composition-time input for the bounds, compaction policy, and JSON encoding of one grant-store leaf.</summary>
/// <remarks>
/// Instances exist only while a configure delegate passed to
/// <see cref="ServiceExtensions.AddJsonSecurityGrantStore(IServiceCollection, JsonSecurityGrantStoreTarget, Action{JsonSecurityGrantStoreOptions}?)"/>
/// runs. The registration materializes the mutated values into an immutable, eagerly validated
/// <see cref="JsonSecurityGrantStoreSettings"/> and never registers this type in dependency injection, so the captured
/// bounds and encoding contract cannot drift after composition. The type performs no validation of its own; invalid values
/// are rejected by the settings constructor with <see cref="ArgumentOutOfRangeException"/> at registration.
/// </remarks>
public sealed class JsonSecurityGrantStoreOptions
{
    /// <summary>Gets or sets the maximum encoded transition-record size.</summary>
    /// <value>A positive byte count enforced before an append and while replaying a log. Defaults to one mebibyte.</value>
    ...
    /// <value>A positive count that bounds replay cost as the store accumulates transitions. Defaults to 4096.</value>
