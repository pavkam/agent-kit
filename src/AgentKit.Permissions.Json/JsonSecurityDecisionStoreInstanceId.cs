...
/// <summary>Identifies the exact initialized JSON decision-store root expected by host bootstrap configuration.</summary>
/// <remarks>
...
/// serving another deployment's decision decisions. Approvals are human authority evidence, so an accidentally shared or
/// swapped root must fail closed rather than let one deployment resolve another's pending requests.
/// </remarks>
public readonly record struct JsonSecurityDecisionStoreInstanceId
{
    ...
    public JsonSecurityDecisionStoreInstanceId(Guid value)
    {
