...
/// <summary>Captures one fixed local JSON store root supplied through the decision control-plane bootstrap boundary.</summary>
/// <remarks>
...
/// </remarks>
public sealed record JsonSecurityDecisionStoreTarget
{
    ...
    /// </remarks>
    public JsonSecurityDecisionStoreTarget(
        string directoryPath,
        JsonSecurityDecisionStoreInstanceId expectedStoreInstanceId,
        ...
        JsonStoreRecoveryMode recoveryMode)
    {
        ...
        if (!Path.IsPathFullyQualified(directoryPath))
        {
            throw new ArgumentException(
                "The JSON decision-store root must be a fully qualified path.", nameof(directoryPath));
        }
    ...
    /// <value>The exact nondefault identity verified against the manifest before every decision-store access.</value>
    public JsonSecurityDecisionStoreInstanceId ExpectedStoreInstanceId { get; }
    ...
    /// <value>The explicit validation or recovery mode applied to the decision record log during initialization.</value>
