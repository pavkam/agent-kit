...
/// <summary>Mutable composition-time input for the bounds, compaction policy, and JSON encoding of one approval-store leaf.</summary>
/// <remarks>
/// Instances exist only while a configure delegate passed to
/// <see cref="ServiceExtensions.AddJsonApprovalStore(IServiceCollection, JsonApprovalStoreTarget, Action{JsonApprovalStoreOptions}?)"/>
/// runs. The registration materializes the mutated values into an immutable, eagerly validated
/// <see cref="JsonApprovalStoreSettings"/> and never registers this type in dependency injection, so the captured bounds and
...
/// </remarks>
public sealed class JsonApprovalStoreOptions
{
    ...
    /// <value>A positive count that bounds replay cost as the store accumulates approval transitions. Defaults to 4096.</value>
