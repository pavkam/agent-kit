...
public sealed partial class JsonApprovalStore
{
        ...
        ApprovalScopeBinding? binding = null)
    {
            ...
            or FormatException or InvalidCastException or OverflowException or ArgumentException)
        {
            var mapped = Unavailable(_corruptEvidence,
                "Persisted JSON approval evidence is corrupt or unsupported.", exception);
            ...
            or NotSupportedException)
        {
            var mapped = Unavailable(_persistenceFailed,
                "The JSON approval-store root could not be written or read.", exception);
            ObserveTerminal(activity.Activity, operation, "unavailable", false, mapped,
                requestId, responseId, measured, startedAt);
            throw mapped;
        }
        ...
        long startedAt)
    {
        ...
        try
        {
            JsonApprovalStoreMetrics.Operations.Add(1,
            ...
            if (measured)
            {
                JsonApprovalStoreMetrics.Duration.Record(
        ...
        try
        {
            ...
            if (successful)
            {
                JsonApprovalStoreLog.OperationCompleted(_logger, operation, outcome, requestText, responseText);
            }
            else
            {
                JsonApprovalStoreLog.OperationFailed(_logger, operation, outcome,
