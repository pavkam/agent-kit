...
/// <summary>Captures immutable positive evidence bounds, compaction policy, and encoding contract for one JSON decision store.</summary>
public sealed record JsonSecurityDecisionStoreSettings
{
    ...
    public JsonSecurityDecisionStoreSettings(
    ...
    /// <value>A positive byte count enforced before an append and while replaying the log. A request carries its complete normalized security request and approver-visible presentation text, so this bound is what keeps one oversized decision from making the whole log unreadable.</value>
    ...
    public static JsonSecurityDecisionStoreSettings CreateDefault() => new(
        1_048_576,
        1_048_576,
        4_096,
