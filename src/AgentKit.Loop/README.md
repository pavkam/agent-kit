...
`AddAgentLoop` registers `DefaultAgentLoop` as a keyed, scoped `IAgentLoop`
selected by an explicit `ComponentKey<IAgentLoop>`, so different agent
definitions in one engine can select different keyed loops. `DefaultAgentLoop`'s
