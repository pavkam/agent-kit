...
internal static partial class HumanQuestionBrokerLog
{
    ...
    [LoggerMessage(EventId = 1003, Message = "Human question {QuestionId} publication completed with {Outcome}. Tenant {TenantId} agent {AgentId} session {SessionId} run {RunId} turn {TurnId} tool {ToolCallId} operation {OperationId} security request {SecurityRequestId}.")]
    ...
    [LoggerMessage(EventId = 1004, Level = LogLevel.Information, Message = "Human question {QuestionId} publication wait was cancelled. Tenant {TenantId} agent {AgentId} session {SessionId} run {RunId} turn {TurnId} tool {ToolCallId} operation {OperationId} security request {SecurityRequestId}.")]
    ...
    [LoggerMessage(EventId = 1005, Level = LogLevel.Error, Message = "Human question {QuestionId} publication failed with error type {ErrorType}. Tenant {TenantId} agent {AgentId} session {SessionId} run {RunId} turn {TurnId} tool {ToolCallId} operation {OperationId} security request {SecurityRequestId}.")]
