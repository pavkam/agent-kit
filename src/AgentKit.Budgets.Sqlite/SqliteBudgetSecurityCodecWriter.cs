...
internal sealed class SqliteBudgetSecurityCodecWriter
{
    ...
    /// <param name="settings">The non-null adapter settings supplying the evidence envelope configuration.</param><param name="maximumBytes">The positive envelope byte limit.</param><param name="paramName">The public evidence parameter reported when the limit is exceeded.</param>
    ...
    internal SqliteBudgetSecurityCodecWriter(SqliteBudgetLedgerSettings settings, int maximumBytes, string paramName)
    {
    ...
    internal void WriteResources(ImmutableArray<ProtectedResource> resources)
    {
        ...
        ArgumentException.ThrowIfContainsNull(resources);
    ...
    {
        Debug.Assert(evidence is not null, "Validated codec inputs are required.");
