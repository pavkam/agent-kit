...
{
    /// <inheritdoc/>
    public ComponentId SecurityAudience { get; } = new("agentkit.session.in-memory");
    ...
    public ValueTask<SessionLaneStateResult> LoadLaneStateAsync(AuthorizedSessionStoreRequest<SessionLaneStateRequest> request, CancellationToken cancellationToken = default) =>
        ObserveAuthorizedAsync(request, SecurityOperationKind.StateRead, SecurityEffect.Observe,
            token => LoadLaneStateCoreAsync(request.Request, token),
    ...
    public ValueTask<SessionRunAbortResult> AbortRunAsync(AuthorizedSessionStoreRequest<SessionRunAbortRequest> request, CancellationToken cancellationToken = default) =>
        ObserveAuthorizedAsync(request, SecurityOperationKind.StateMutation, SecurityEffect.Mutate,
            token => AbortRunCoreAsync(request.Request, token), static result => result is SessionRunAbortRecorded,
        ...
        where TRequest : class where TResult : class
    {
        ...
        return SessionStoreObservability.ObserveAsync(
            ...
            async token =>
            {
                ...
                return denial is null ? await action(token).ConfigureAwait(false) : rejected(denial);
