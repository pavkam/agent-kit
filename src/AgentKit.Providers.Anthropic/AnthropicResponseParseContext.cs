...
/// <see cref="IAnthropicMessageStreamParser"/> needs to translate one raw
/// Anthropic Messages HTTP response body into a
...
/// </remarks>
public sealed record AnthropicResponseParseContext
{
    /// <summary>Initializes a new instance of the <see cref="AnthropicResponseParseContext"/> record.</summary>
    ...
    /// <c>request-id</c> HTTP response header, when the provider supplied
    /// one.
    /// </param>
    public AnthropicResponseParseContext(
    ...
    /// the <c>request-id</c> HTTP response header, when the provider
    /// supplied one.
    /// </summary>
