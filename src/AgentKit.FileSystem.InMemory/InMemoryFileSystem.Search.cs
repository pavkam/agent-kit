...
public sealed partial class InMemoryFileSystem
{
    ...
    /// <inheritdoc/>
    private async ValueTask<FileSearchResult> SearchCoreAsync(FileSearchRequest request, CancellationToken cancellationToken)
    {
        ...
        lock (_gate)
        {
            ...
            if (!DirectoryExists(basePath))
            {
                return _files.ContainsKey(basePath ?? "")
                    ? Failure(FileSearchStatus.Denied, "The search base crosses an inaccessible boundary.")
            ...
            return state.TerminalStatus is { } terminal
                ...
                : matches.IsEmpty
                    ? new FileSearchResult(
                        FileSearchStatus.NoMatches, [], state.VisitedFiles, state.VisitedBytes, true,
    ...
    private void SearchFile(string relative, SearchTraversalState state)
    {
        ...
        {
            array = [.. content];
            _ = _strictUtf8.GetCharCount(array);
        }
        ...
        var fingerprint = new ContentHash($"sha256:{Convert.ToHexString(SHA256.HashData(array)).ToLowerInvariant()}");
        MatchFileLines(array, new FileSystemPath(workspacePath), fingerprint, state);
    }
        ...
        SearchTraversalState state)
    {
        ...
        while (lineStart <= bytes.Length)
        {
            ...
            foreach (var (characterOffset, characterLength) in state.FindMatches(decodedLine))
            {
                ...
                var matchByteLength = _strictUtf8.GetByteCount(decodedLine.AsSpan(characterOffset, characterLength));
                ...
                while (projectionOffset < matchByteOffset && (lineBytes[projectionOffset] & 0xC0) == 0x80)
                {
                    projectionOffset++;
                }

                var projectionLength = Math.Min(lineBytes.Length - projectionOffset, state.Request.MaximumLineBytes);
    ...
    private static FileSearchResult Failure(FileSearchStatus status, string message, bool complete = false) =>
        new(status, [], 0, 0, complete, message);
