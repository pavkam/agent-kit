...
/// <remarks>
/// The dispatcher snapshots registrations and delivery policy at construction. It checks caller cancellation before
/// and after each sink completion, so it never begins a later sink invocation after cancellation becomes known.
/// A sink that already accepted a record remains an external durable fact when cancellation propagates; the dispatcher
/// does not reclassify that record or attempt compensating deletion. It performs no authorization, grant, or audit
/// persistence of its own, so audit infrastructure cannot recursively authorize itself.
/// </remarks>
internal sealed class DefaultSecurityAuditDispatcher: ISecurityAuditDispatcher
{
    ...
    /// <exception cref="ArgumentOutOfRangeException">The captured audit delivery is undefined.</exception>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            foreach (var binding in applicable)
            {
                ...
                try
                {
                    await binding.Sink.WriteAsync(record, cancellationToken).ConfigureAwait(false);
