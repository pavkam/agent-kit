...
public static class ServiceExtensions
{
    extension(IServiceCollection services)
    {
            ...
            ModelLimits? limits = null)
        {
            ...
            return services.AddMoonshotKimiLlmModel(new ModelDescriptor(
                alias,
                MoonshotKimiProviderDefaults.ProviderId,
        ...
        /// <summary>
        /// Registers one Moonshot Kimi conversational model from a complete descriptor as an
        /// additional <see cref="ILlmModel"/> implementation.
        /// </summary>
        /// <param name="descriptor">
        /// The exact descriptor the adapter will serve; it must name the Moonshot Kimi provider and API family.
        /// </param>
        ...
        /// rebuilding an equivalent one. Additive; <see cref="AddMoonshotKimi"/> must be called first.
        /// </remarks>
        public IServiceCollection AddMoonshotKimiLlmModel(ModelDescriptor descriptor)
        {
            ArgumentNullException.ThrowIfNull(services);
            ArgumentNullException.ThrowIfNull(descriptor);
            if (descriptor.ProviderId != MoonshotKimiProviderDefaults.ProviderId || descriptor.ApiFamily != MoonshotKimiProviderDefaults.ApiFamily)
            {
        ...
        /// <summary>
        /// Registers one Moonshot Kimi conversational model from the bundled <see cref="KnownModelCatalog"/>: both the
        ...
        /// </summary>
        /// <param name="alias">The application-facing selection key for this model.</param>
        /// <param name="modelId">The vendor's own model identifier; it must exist in the catalog under the Moonshot Kimi provider.</param>
        ...
        /// <remarks>
        /// <para>
        /// This is the one-call form of <c>AddMoonshotKimiLlmModel(ModelDescriptor)</c> followed by <c>AddModelDescriptors</c>
        /// with an identical descriptor. Both registrations are additive; the descriptor source is keyed
        /// <c>moonshotkimi.known/{alias}</c>. <see cref="AddMoonshotKimi"/> must be called first and <c>AddAgentProviders</c>
        ...
        /// know can still be registered explicitly with <c>AddMoonshotKimiLlmModel(alias, modelId, capabilities, limits)</c>
        /// followed by <c>AddModelDescriptors</c>.
        /// </para>
        /// </remarks>
        public IServiceCollection AddMoonshotKimiKnownLlmModel(
            ModelAlias alias,
            ModelId modelId,
            KnownModelCatalog? catalog = null)
        {
            ...
            if (!catalog.TryFind(MoonshotKimiProviderDefaults.ProviderId, modelId, out var known))
            {
                throw new ArgumentException(
                    $"'{modelId.Value}' is not a Moonshot Kimi model in the known-model catalog; register it explicitly with {nameof(AddMoonshotKimiLlmModel)}.",
                    nameof(modelId));
            }

            var descriptor = known.ToDescriptor(alias, MoonshotKimiProviderDefaults.ApiFamily, MoonshotKimiProviderDefaults.DefaultCapabilities);
            _ = services.AddMoonshotKimiLlmModel(descriptor);
            _ = services.AddModelDescriptors(new ModelDescriptorSourceId($"moonshotkimi.known/{alias.Value}"), [descriptor]);
            return services;
        }
    }
}
