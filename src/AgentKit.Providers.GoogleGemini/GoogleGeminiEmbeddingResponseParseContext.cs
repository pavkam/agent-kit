...
/// <see cref="IGoogleGeminiEmbeddingResponseParser"/> needs to translate one
/// raw Gemini <c>batchEmbedContents</c> HTTP response body into a terminal
...
/// each vector's embedding space: Gemini uses asymmetric transforms per task
/// type (<c>RETRIEVAL_QUERY</c>, <c>RETRIEVAL_DOCUMENT</c>,
/// <c>CLASSIFICATION</c>, ...), so vectors computed under different task
/// types are not comparable and must carry distinct
/// <see cref="EmbeddingSpaceIdentity"/> values. Gemini's <c>batchEmbedContents</c>
/// endpoint exposes no deployment concept, so the inherited
/// <see cref="EmbeddingResponseParseContext.DeploymentId"/> is always
...
/// </remarks>
public sealed record GoogleGeminiEmbeddingResponseParseContext: EmbeddingResponseParseContext
{
    ...
    /// The purpose the request declared as <c>taskType</c>, echoed onto each
    /// resulting <see cref="EmbeddingSpaceIdentity"/> because Gemini's
    ...
    public GoogleGeminiEmbeddingResponseParseContext(
        ...
        : base(requestId, providerId, apiFamily, requestedModelId, deploymentId: null, providerRequestId) =>
        RequestedPurpose = requestedPurpose;

    /// <summary>
    /// Gets the purpose the request declared as <c>taskType</c>, echoed onto
    /// each resulting <see cref="EmbeddingSpaceIdentity"/> because Gemini's
    /// asymmetric query/document embedding spaces are not comparable across
    /// purposes.
    /// </summary>
